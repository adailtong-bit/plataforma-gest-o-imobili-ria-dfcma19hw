...
//     USING: ((property_id IN ( SELECT properties.id    FROM properties   WHERE ((properties.owner_id = auth.uid()) OR (properties.agent_id = auth.uid())))) OR (assignee_id = auth.uid()) OR (created_by = auth.uid()) OR is_admin_or_pm())
//     WITH CHECK: ((property_id IN ( SELECT properties.id    FROM properties   WHERE ((properties.owner_id = auth.uid()) OR (properties.agent_id = auth.uid())))) OR (assignee_id = auth.uid()) OR (created_by = auth.uid()) OR is_admin_or_pm())
// Table: towers
