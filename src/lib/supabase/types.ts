...
//     WITH CHECK: true
// Table: conversations
//   Policy "Participants can view conversations" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: ((id IN ( SELECT conversation_participants.conversation_id    FROM conversation_participants   WHERE (conversation_participants.profile_id = auth.uid()))) OR is_admin_or_pm())
...
//     USING: ((id = auth.uid()) OR (pm_id = auth.uid()) OR (pm_id = ( SELECT p2.pm_id    FROM profiles p2   WHERE (p2.id = auth.uid()))) OR (id = ( SELECT p2.pm_id    FROM profiles p2   WHERE (p2.id = auth.uid()))) OR is_admin_or_pm() OR (id IN ( SELECT conversation_participants.profile_id    FROM conversation_participants   WHERE (conversation_participants.conversation_id IN ( SELECT conversation_participants_1.conversation_id            FROM conversation_participants conversation_participants_1           WHERE (conversation_participants_1.profile_id = auth.uid()))))))
