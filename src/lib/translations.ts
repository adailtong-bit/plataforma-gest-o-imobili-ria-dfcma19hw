export type Language = 'pt' | 'en' | 'es'

export const translations = {
  pt: {
    sync: {
      title: 'Sincronização Básica iCal',
      sync_desc: 'Sincronização simples de calendário via URL.',
      platform: 'Plataforma',
      ical_url: 'URL iCal',
      add_link: 'Adicionar Link',
      status: 'Status',
      last_sync: 'Última Sincronização',
      no_links: 'Nenhum link configurado.',
      sync_now: 'Sincronizar Agora',
      sync_success: 'Sincronização Iniciada',
      export_calendar: 'Exportar Calendário',
      copy_link: 'Copiar Link',
      airbnb: 'Airbnb (iCal)',
      booking: 'Booking.com (iCal)',
      vrbo: 'Vrbo (iCal)',
    },
    publicity: {
      title: 'Administração de Publicidade',
      subtitle:
        'Gerencie anunciantes, campanhas e configurações de monetização.',
      tab_ads: 'Anúncios',
      tab_advertisers: 'Anunciantes',
      tab_pricing: 'Config. de Preços',
      advertiser_list: {
        title: 'Anunciantes',
        search_placeholder: 'Buscar anunciantes...',
        add_btn: 'Adicionar Anunciante',
        table_company: 'Empresa / Nome',
        table_contact: 'Contato',
        table_address: 'Endereço',
        table_registered: 'Registrado',
        empty_state: 'Nenhum anunciante encontrado.',
        modal_edit: 'Editar Anunciante',
        modal_new: 'Novo Anunciante',
        label_company: 'Empresa / Nome *',
        label_email: 'Email *',
        label_phone: 'Telefone',
        label_address: 'Endereço',
        delete_confirm: 'Tem certeza que deseja excluir este anunciante?',
        delete_success: 'Anunciante excluído.',
        update_success: 'Anunciante atualizado com sucesso.',
        add_success: 'Anunciante adicionado com sucesso.',
        validation_error: 'Nome e email são obrigatórios.',
      },
      ads_manager: {
        title: 'Gerenciador de Anúncios',
        search_placeholder: 'Buscar anúncios...',
        add_btn: 'Novo Anúncio',
        table_ad_info: 'Info do Anúncio',
        table_advertiser: 'Anunciante',
        table_placement: 'Posicionamento',
        table_validity: 'Validade',
        table_price: 'Preço',
        table_status: 'Status',
        empty_state: 'Nenhum anúncio encontrado.',
        modal_edit: 'Editar Anúncio',
        modal_new: 'Novo Anúncio',
        label_title: 'Título *',
        label_advertiser: 'Anunciante *',
        select_advertiser: 'Selecione o anunciante',
        label_placement: 'Local de Posicionamento',
        label_price: 'Preço',
        label_start_date: 'Data de Início',
        label_end_date: 'Data de Término',
        label_image_url: 'URL da Imagem *',
        label_target_link: 'URL do Link de Destino',
        active_status: 'Status Ativo',
        active_status_desc: 'Alternar a visibilidade do anúncio na plataforma.',
        save_ad: 'Salvar Anúncio',
        delete_confirm: 'Tem certeza que deseja excluir este anúncio?',
        delete_success: 'Anúncio excluído.',
        update_success: 'Anuncio atualizado com sucesso.',
        add_success: 'Anúncio criado com sucesso.',
        validation_error:
          'Título, URL da Imagem e Anunciante são obrigatórios.',
        link: 'Link',
        active: 'Ativo',
        inactive: 'Inativo',
        unknown: 'Desconhecido',
        placements: {
          home_top: 'Banner Superior Início',
          home_bottom: 'Banner Inferior Início',
          partner_page: 'Página do Parceiro',
          tenant_page: 'Portal do Inquilino',
          pm_login: 'Tela de Login do Gerente',
          sidebar: 'Barra Lateral',
          footer: 'Rodapé',
        },
      },
      pricing_config: {
        title_base: 'Taxas Base por Duração',
        desc_base:
          'Defina os preços padrão para diferentes durações de anúncios.',
        label_weekly: 'Taxa Semanal',
        label_biweekly: 'Taxa Quinzenal (14 Dias)',
        label_monthly: 'Taxa Mensal',
        title_modifiers: 'Modificadores de Posicionamento',
        desc_modifiers: 'Custos adicionais para posicionamentos premium.',
        tooltip_modifiers:
          'Estes valores representam adições fixas à taxa base de duração, dependendo de onde o anúncio é posicionado.',
        save_btn: 'Salvar Configuração',
        save_success_title: 'Configuração de Preços Salva',
        save_success_desc:
          'Suas taxas de anúncios foram atualizadas com sucesso.',
      },
    },
    sidebar: {
      dashboard: 'Painel',
      portfolio: 'Portfólio',
      units: 'Propriedades',
      condominiums: 'Condomínios',
      owners: 'Proprietários',
      tenants: 'Inquilinos',
      financial: 'Financeiro',
      marketing: 'Marketing',
      reports: 'Relatórios',
      settings: 'Configurações',
      performance: 'Desempenho',
      guest_services: 'Serviços Hóspede',
      pos: 'Ponto de Venda',
      main_menu: 'Menu Principal',
      system: 'Sistema',
      migration_hub: 'Migração',
      publicity_admin: 'Publicidade',
      users: 'Usuários',
      tenant_portal: 'Portal Inquilino',
      owner_portal: 'Portal Proprietário',
      partner_portal: 'Portal Parceiro',
      housekeeping: 'Limpeza',
      night_audit: 'Auditoria Noturna',
      calendar: 'Calendário',
      partners: 'Parceiros',
    },
    housekeeping: {
      title: 'Limpeza / Housekeeping',
      subtitle: 'Gerencie tarefas de limpeza e agendamentos.',
      task_title: 'Título da Tarefa',
      property: 'Propriedade',
      assignee: 'Responsável',
      date: 'Data',
      status: 'Status',
    },
    hotel_form: {
      hotel_name: 'Nome do Hotel',
      street_address: 'Endereço',
      number: 'Número',
      neighborhood: 'Bairro',
      city: 'Cidade',
      state: 'Estado / Província',
      zip_code: 'CEP / Código Postal',
      towers_wings: 'Torres / Alas',
      add_division: 'Adicionar Divisão',
      define_divisions: 'Defina divisões internas como Torres ou Alas.',
      validation_error: 'Nome e Cidade são obrigatórios.',
      placeholder_name: 'Ex: Grand Heritage Hotel',
      details: 'Detalhes',
      location: 'Localização',
      no_divisions:
        'Nenhuma divisão adicionada. Clique em "Adicionar Divisão" para criar uma.',
      delete_title: 'Excluir Hotel',
      delete_desc:
        'Esta ação não pode ser desfeita. Tem certeza que deseja excluir este hotel permanentemente?',
    },
    hotels: {
      title: 'Hotéis',
      subtitle: 'Gestão de hotéis e quartos.',
      add_title: 'Adicionar Hotel',
      new_hotel: 'Novo Hotel',
      towers: 'Torres',
      rooms: 'Quartos',
      floors: 'Andares',
      add_tower: 'Adicionar Torre',
      new_tower: 'Nova Torre',
      hotel_details: 'Detalhes do Hotel',
      no_hotels: 'Nenhum hotel encontrado.',
      not_found: 'Hotel Não Encontrado',
      update_success: 'Hotel atualizado com sucesso.',
      delete_success: 'Hotel excluído.',
      tower_add_success: 'Torre adicionada com sucesso.',
      all_rooms: 'Todos os Quartos',
      info: 'Informações do Hotel',
      towers_wings: 'Torres e Alas',
      add_new_tower: 'Adicionar Nova Torre/Ala',
      tower_name: 'Nome da Torre',
      manage_rooms: 'Gerenciar Quartos',
      no_towers: 'Nenhuma torre ou ala definida.',
    },
    header: {
      demo_profiles: 'Perfiles Demo',
      other_users: 'Outros Usuários',
    },
    market_analysis: {
      title: 'Análise de Mercado',
      subtitle: 'Benchmark de mercado e tendências.',
      demand_index: 'Índice de Demanda',
      peak_season: 'Alta temporada aproximando',
      comp_set_rank: 'Classificação do Set Competitivo',
      rank_desc: 'De 12 hotéis locais',
      competitor_pricing: 'Preços da Concorrência',
      demand_forecast: 'Previsão de Demanda',
      no_forecast: 'Nenhuma previsão disponível.',
      avg_occupancy: 'Taxa de Ocupação Média',
      avg_daily_rate: 'Tarifa Média Diária (ADR)',
      spending_trends: 'Tendências de Gastos',
      high: 'Alta',
      medium: 'Média',
      low: 'Baixa',
      historical_view: 'Visão histórica do desempenho do mercado',
      avg_rate_comparison: 'Comparação de diária média',
      projected_demand: 'Níveis de demanda projetados para datas futuras',
    },
    pos: {
      title: 'Ponto de Venda',
      subtitle: 'Gestão de produtos, vendas e estoque.',
      new_product: 'Novo Produto',
      manage_products: 'Gerenciar Produtos',
      product_name: 'Nome do Produto',
      price: 'Preço',
      category: 'Categoria',
      validity_start: 'Validade Inicial',
      active: 'Ativo',
      inactive: 'Inativo',
      stock: 'Estoque',
      actions: 'Ações',
      create: 'Criar',
      edit: 'Alterar',
      inactivate: 'Inativar',
      delete: 'Excluir',
      confirm_delete: 'Confirmar Exclusão',
      delete_desc: 'Tem certeza que deseja excluir este produto?',
      save_success: 'Produto salvo com sucesso.',
      add_to_cart: 'Adicionar ao Carrinho',
      checkout: 'Finalizar Venda',
      charge_room: 'Cobrar no Quarto',
      out_of_stock: 'Sem Estoque',
      cart_empty: 'Carrinho Vazio',
      transaction_success: 'Venda realizada com sucesso!',
      laundry_service: 'Serviço de Lavanderia',
      laundry: 'Lavanderia',
      club_sandwich: 'Club Sandwich',
      restaurant: 'Restaurante',
      minibar: 'Minibar',
      cola: 'Refrigerante',
      chips: 'Batata Chips',
      guest: 'Hóspede',
      select: 'Selecione',
    },
    guest_services: {
      title: 'Serviços ao Hóspede',
      subtitle: 'Catálogo de serviços e agendamentos.',
      new_service: 'Novo Serviço',
      manage_services: 'Gerenciar Serviços',
      financial_insights: 'Insights Financeiros',
      service_name: 'Nome do Serviço',
      description: 'Descrição',
      price: 'Preço',
      category: 'Categoria',
      validity_start: 'Validade Inicial',
      seasonal_pricing: 'Preços por Temporada',
      add_seasonal_price: 'Adicionar Preço de Temporada',
      start_date: 'Data Início',
      end_date: 'Data Fim',
      schedule: 'Agendar',
      schedule_for: 'Agendar Para',
      assign_service: 'Atribuir Serviço',
      service_assigned: 'Serviço atribuído com sucesso.',
      save_success: 'Serviço salvo com sucesso.',
    },
    performance: {
      guest_reviews: 'Avaliações dos Hóspedes',
      average_rating: 'Avaliação Média',
      based_on_reviews: 'Baseado em {count} avaliações',
      response: 'Resposta',
      no_reviews: 'Nenhuma avaliação ainda.',
      rating_reviews_trend: 'Tendência de Avaliações e Comentários',
      guest_satisfaction_evolution:
        'Evolução da satisfação dos hóspedes ao longo do tempo.',
      financial_performance: 'Desempenho Financeiro (RevPAR)',
      revpar_trends: 'Tendências de Receita por Quarto Disponível.',
    },
    marketing: {
      promotions: 'Promoções',
      campaigns: 'Campanhas',
      automation: 'Automação',
      leads: 'Leads & Interessados',
      new_code: 'Novo Código',
      create_promo: 'Criar Código Promocional',
      discount: 'Desconto',
      validity: 'Validade',
      usage: 'Uso',
      new_campaign: 'Nova Campanha',
      create_campaign: 'Criar Campanha',
      target_audience: 'Público Alvo',
      code: 'Código',
      automated_workflows: 'Fluxos Automatizados',
      automated_desc: 'Disparar emails baseados em eventos.',
      email_templates: 'Modelos de Email',
      email_desc: 'Conteúdo reutilizável com variáveis.',
      trigger_event: 'Evento Gatilho',
      timing: 'Timing',
      template: 'Modelo',
      subject: 'Assunto',
      body: 'Corpo',
      placeholders: 'Variáveis disponíveis',
      active: 'Ativo',
      booking_confirmed: 'Reserva Confirmada',
      check_in: 'Check-in',
      check_out: 'Check-out',
      cancellation: 'Cancelamento',
      immediately: 'Imediatamente',
      hours: 'horas',
      before: 'antes',
      after: 'depois',
      new_message: 'Nova Mensagem',
      new_workflow: 'Novo Workflow',
    },
    analytics_page: {
      occupancy_comparison: 'Comparação de Ocupação',
      profitability_comparison: 'Comparação de Rentabilidade',
      internal_performance: 'Performance Interna',
      market_average: 'Média de Mercado',
      benchmark_desc: 'Comparativo de desempenho com o mercado.',
    },
    reports: {
      title: 'Relatórios Avançados',
      subtitle:
        'Análise de inventário, manutenção e condições das propriedades.',
      period: 'Período',
      property: 'Propriedade',
      apply_filters: 'Aplicar Filtros',
      maintenance_analytics: 'Análise de Manutenção',
      inventory_health: 'Saúde do Inventário',
      maintenance_efficiency: 'Eficiência de Manutenção',
      inventory_condition: 'Condição do Inventário',
      damage_by_property: 'Danos por Propriedade',
      total_items: 'Total de Itens',
      damaged_poor: 'Danificado/Ruim',
      total_spend: 'Gasto Total',
      total_tasks: 'Total de Tarefas',
      avg_resolution: 'Resolução Média',
      avg_cost_task: 'Custo Médio/Tarefa',
      cost_breakdown: 'Detalhamento de Custos',
      distribution_expenses: 'Distribuição de Despesas',
      monthly_expenditure: 'Despesa Mensal',
      spending_trends: 'Tendências de Gastos',
      reports: 'Relatórios',
    },
    service_pricing: {
      title: 'Catálogo de Preços',
      desc: 'Catálogo de preços para serviços e produtos genéricos.',
      subtitle: 'Gerencie custos e preços de venda.',
      catalog_tab: 'Catálogo de Preços',
      analytics_tab: 'Relatórios',
      new_rate: 'Nova Taxa',
      service_name: 'Nome do Serviço',
      partner_payment: 'Pagamento Parceiro',
      pm_value: 'Valor PM',
      product_price: 'Preço de Venda',
      cost_price: 'Preço de Custo',
      valid_from: 'Válido De',
      valid_to: 'Válido Até',
      margin: 'Margem',
      distribution: 'Distribuição',
      profit_analysis: 'Análise de Lucro',
    },
    condominiums: {
      title: 'Condomínios',
      subtitle: 'Gestão de condomínios e associações.',
      new_condo: 'Novo Condomínio',
      add_title: 'Adicionar Condomínio',
      manager: 'Gerente/Síndico',
      contact: 'Contato',
      no_condos: 'Nenhum condomínio encontrado.',
      general_info: 'Informações Gerais',
      addressing: 'Endereçamento',
      access_codes: 'Códigos de Acesso',
      management: 'Gestão',
      description: 'Descrição',
      street: 'Rua/Avenida',
      number: 'Número',
      complement: 'Número ou Complemento',
      neighborhood: 'Bairro',
      city: 'Cidade',
      state: 'Estado',
      zip_code: 'CEP',
      manager_name: 'Nome do Gerente',
      manager_phone: 'Telefone do Gerente',
      manager_email: 'Email do Gerente',
      gate_code: 'Código do Portão',
      pedestrian_gate: 'Portão de Pedestres',
      pool_code: 'Código da Piscina',
      amenities_code: 'Código das Áreas Comuns',
      edit_condo: 'Editar Condomínio',
      view_details: 'Ver Detalhes',
      search_placeholder: 'Buscar condomínio...',
      delete_confirm: 'Tem certeza que deseja excluir este condomínio?',
    },
    owners: {
      title: 'Proprietários',
      subtitle: 'Base de proprietários e investidores.',
      new_owner: 'Novo Proprietário',
      register_title: 'Cadastro de Proprietário',
      base_title: 'Base de Proprietários',
      search_placeholder: 'Buscar proprietário...',
      contact_details: 'Detalhes de Contato',
      properties_count: 'Propiedades',
      workflow_started: 'Workflow Iniciado',
      workflow_desc: 'Ação {action} iniciada para {name}.',
      renew_contract: 'Renovar Contrato',
      success_desc: 'Proprietário registrado com sucesso.',
      add_owner: 'Incluir Proprietário',
      edit_owner: 'Alterar Proprietário',
      owner_added: 'Proprietário incluído com sucesso',
      owner_updated: 'Proprietário alterado com sucesso',
      delete_owner: 'Excluir Proprietário',
      delete_owner_desc: 'Esta ação não pode ser desfeita.',
    },
    tenants: {
      title: 'Inquilinos',
      subtitle: 'Gestão de contratos e inquilinos.',
      new_tenant: 'Novo Inquilino',
      register_title: 'Cadastro de Inquilino',
      list_title: 'Lista de Inquilinos',
      search_placeholder: 'Buscar inquilino...',
      property: 'Propriedade',
      send_message: 'Enviar Mensagem',
      add_tenant: 'Incluir Inquilino',
      edit_tenant: 'Alterar Inquilino',
      tenant_added: 'Inquilino incluído com sucesso',
      tenant_updated: 'Inquilino alterado com sucesso',
      delete_tenant: 'Excluir Inquilino',
      delete_tenant_desc: 'Esta ação não pode ser desfeita.',
    },
    partners: {
      title: 'Parceiros',
      subtitle: 'Fornecedores e prestadores de serviço.',
      new_partner: 'Novo Parceiro',
      register_title: 'Cadastro de Parceiro',
      no_partners: 'Nenhum parceiro encontrado.',
      search_placeholder: 'Buscar parceiro...',
      contact_name: 'Nome do Contato',
      company_name: 'Nome da Empresa',
      category: 'Categoria',
      agent: 'Agente',
      cleaning: 'Limpeza',
      maintenance: 'Manutenção',
      bank_info: 'Dados Bancarios',
      bank_name: 'Banco',
      routing: 'Agência/Routing',
      account: 'Conta',
      payment_history: 'Histórico de Pagamentos',
      financial_report: 'Relatório Financeiro',
      inspection: 'Inspeção',
    },
    invoices: {
      title: 'Faturas',
      subtitle: 'Gestão de cobranças e pagamentos.',
      create_new: 'Criar Nova',
      invoice_id: 'ID Fatura',
      amount: 'Valor',
      bill_to: 'Faturar Para',
      status_paid: 'Pago',
      status_pending: 'Pendente',
      status_approved: 'Aprovado',
      no_invoices: 'Nenhuma fatura encontrada.',
      send_to_payer: 'Enviar ao Pagador',
      sent_success: 'Fatura enviada com sucesso.',
      view: 'Visualizar',
      invoice_viewer: {
        title: 'Visualizador de Fatura',
        invoice_no: 'Fatura Nº',
        date: 'Data',
        bill_to: 'Faturar Para',
        description: 'Descrição',
        quantity: 'Qtd',
        amount: 'Valor',
        subtotal: 'Subtotal',
        total: 'Total',
        thank_you: 'Obrigado pela preferência!',
        print: 'Imprimir',
        download: 'Baixar',
      },
    },
    financial: {
      title: 'Financeiro',
      subtitle: 'Visão geral financeira e conciliação.',
      management_desc: 'Gestão de contas correntes, custos fixos e variáveis.',
      add_transaction: 'Incluir Lançamento',
      add_title: 'Incluir Transação',
      export_csv: 'Exportar (CSV)',
      type: 'Tipo',
      date: 'Data',
      description: 'Descrição',
      category: 'Categoria',
      amount: 'Valor',
      property_optional: 'Propriedade (Opcional)',
      property: 'Propriedade',
      cost_category: 'Categoria de Custo',
      is_recurring: 'É uma despesa/receita recorrente?',
      recurrence_freq: 'Frequência de Repetição',
      recurring_hint:
        'Ao marcar este item como "Pago", o sistema irá gerar automaticamente o lançamento do próximo mês/ano.',
      initial_status: 'Status Inicial',
      save_transaction: 'Salvar Lançamento',
      balances_panel: 'Painel de Saldos e Relatórios',
      balances_desc:
        'Visualize o saldo da conta corrente filtrado pela sua preferência.',
      pm_general: 'PM (Geral)',
      owner: 'Proprietário',
      select_owner: 'Selecione o Proprietário',
      all_owners: 'Todos os Proprietários',
      all_properties: 'Todas as Propriedades',
      viewing_your_properties: 'Visualizando Suas Propriedades',
      filters: 'Filtros:',
      all_categories: 'Todas Categorias',
      incomes: 'Receitas',
      expenses: 'Despesas',
      maintenances: 'Manutenções',
      cleanings: 'Limpezas',
      hoa_condo: 'HOA / Condomínio',
      taxes: 'Impostos',
      all_period: 'Todo o Período',
      current_month: 'Mês Atual',
      semester: 'Semestre',
      fiscal_year: 'Ano Fiscal',
      custom: 'Personalizado',
      projected_balance: 'Saldo Final Projetado',
      projected_balance_desc: 'Saldo projetado incluindo o histórico filtrado',
      total_incomes: 'Total Receitas (No Período)',
      total_expenses: 'Total Despesas (No Período)',
      pending_suffix: 'pendientes',
      statement: 'Extrato de Lançamentos',
      table_date: 'Data',
      table_desc: 'Descrição',
      table_type_cat: 'Tipo / Categoria',
      table_status: 'Status',
      table_amount: 'Valor',
      table_balance: 'Saldo',
      table_actions: 'Ações',
      monthly: 'Mensal',
      yearly: 'Anual',
      income: 'Receita',
      expense: 'Despesa',
      fixed_cost: 'Custo Fixo',
      variable_cost: 'Custo Variável',
      cleared: 'Pago/Recebido',
      pay: 'Pagar',
      edit_transaction: 'Alterar Transação',
      save_changes: 'Salvar Alterações',
      delete_transaction: 'Excluir Transação',
      delete_confirm:
        'Tem certeza que deseja excluir esta transação? Esta ação não pode ser desfeita.',
      cancel: 'Cancelar',
      delete: 'Excluir',
      no_transactions:
        'Nenhuma transação encontrada para os filtros selecionados.',
      success_add: 'Transação incluída com sucesso',
      success_edit: 'Transação alterada com sucesso',
      success_delete: 'Transação excluída com sucesso',
      success_paid: 'Transação marcada como paga',
      invalid_amount: 'Valor inválido',
      invalid_amount_desc: 'O valor deve ser maior que zero.',
      invalid_desc: 'Descrição inválida',
      invalid_desc_hint: 'A descrição é obrigatória.',
      reconciliation: 'Conciliação',
      upload_statement: 'Carregar Extrato',
      upload_desc: 'Faça upload de extratos bancários para conciliação.',
      statements: 'Extratos',
      no_statements: 'Nenhum extrato carregado.',
      reconciled: 'Conciliado',
      pending_reconciliation: 'Pendente Conciliação',
      overview_pnl: 'Visão Geral & DRE',
      tower_breakdown: 'Detalhamento por Torre',
      channel_analytics: 'Análise de Canais',
      projected_cash_flow: 'Fluxo de Caixa Projetado',
      total_expenses: 'Despesas Totais',
      net_income: 'Lucro Líquido',
      owner_statement: 'Extrato do Proprietário',
      gross_revenue: 'Receita Bruta',
      negative_payout: 'Saldo Devedor / Retenção',
      this_month: 'Mês Atual',
      last_month: 'Mês Passado',
      last_3_months: 'Últimos 3 Meses',
      semester: 'Semestre',
      current_year: 'Ano Atual',
      previous_year: 'Ano Anterior',
      running_balance: 'Saldo Acumulado',
      secure_payment: 'Pagamento Seguro',
      complete_payment_for: 'Concluir pagamento de:',
      card: 'Cartão',
      bank: 'Banco',
      card_number: 'Número do Cartão',
      expiry: 'Validade',
      cvc: 'CVC',
      pay_now: 'Pagar Agora',
      bank_transfer_details:
        'Detalhes da transferência bancária (PIX/TED) aparecerão aqui.',
      simulate_transfer: 'Simular Transferência',
      record_manual_payment: 'Registrar pagamento manual (Dinheiro/Cheque).',
      record_payment: 'Registrar Pagamento',
      payment_successful: 'Pagamento Realizado',
      transaction_id: 'ID da Transação:',
      total_amount_charged: 'Valor Total (Cobrado do Proprietário)',
      partner_cost: 'Custo do Parceiro:',
      calculated_pm_commission: 'Comissão PM Calculada:',
      generate_account: 'Gerar & Contabilizar',
      invoice_generated: 'Fatura Gerada e Contabilizada',
      categories: {
        other: 'Outros',
        rent: 'Aluguel',
        maintenance: 'Manutenção',
        cleaning: 'Limpeza',
        hoa: 'HOA / Condomínio',
        tax: 'Impostos',
        utilities: 'Utilidades (Água, Luz)',
      },
    },
    tasks: {
      title: 'Tarefas',
      subtitle: 'Gestão de manutenção e serviços.',
      board: 'Quadro',
      list: 'Lista',
      in_progress: 'Em Progresso',
      approval: 'Aprovação',
      status_wait_owner: 'Aguardando Proprietário',
      status_wait_pm: 'Aguardando Gerente',
      task_title: 'Título da Tarefa',
      assignee: 'Responsável',
      scheduled_date: 'Data Agendada',
      service_type: 'Tipo de Serviço',
      change_status: 'Mudar Status',
      location: 'Localização',
    },
    visits: {
      schedule_visit: 'Agendar Visita',
      enter_details: 'Insira os detalhes da visita abaixo.',
      assign_team: 'Atribuir Membro da Equipe / Parceiro',
      time: 'Hora',
      reason: 'Motivo da Visita',
      showing: 'Apresentação',
      inspection: 'Inspeção',
      client_prefs: 'Preferências do cliente, códigos de acesso, etc.',
      list_title: 'Lista de Visitas',
      upcoming_past: 'Visitas futuras e passadas.',
      no_visits: 'Nenhuma visita agendada.',
    },
    calendar: {
      title: 'Calendário',
      subtitle: 'Visão integrada de eventos e tarefas.',
      integrated_view: 'Visão Integrada',
      no_activities: 'Nenhuma atividade registrada para hoje.',
    },
    messages: {
      title: 'Mensagens',
      subtitle: 'Central de comunicação unificada.',
      new_message: 'Nova Mensagem',
      select_user: 'Selecione um Usuário',
      group_team: 'Equipe',
      group_tenants: 'Inquilinos',
      group_owners: 'Proprietários',
      group_partners: 'Parceiros',
      type_message: 'Digite sua mensagem...',
      online: 'Online',
      no_contacts: 'Sem contatos.',
      search_contact: 'Buscar contato...',
      no_conv: 'Nenhuma conversa encontrada.',
      new_chat_prompt: 'Iniciar uma nova conversa',
      select_prompt: 'Selecione um contato para começar a conversar.',
      start_chat_desc: 'Iniciar Chat',
    },
    users: {
      title: 'Usuários',
      subtitle: 'Gestão de usuários e permissões do sistema.',
      role_label: 'Função',
      registered: 'Registrados',
      registered_desc: 'usuários registrados no sistema.',
      permissions: 'Permissões',
      status_active: 'Ativo',
      status_pending: 'Pendente',
      status_blocked: 'Bloqueado',
      approve_success: 'Usuário aprovado com sucesso.',
      block_title: 'Bloquear Usuário',
      block_confirm: 'Tem certeza que deseja bloquear este usuário?',
      block_success: 'Usuário bloqueado com sucesso.',
    },
    workflows: {
      title: 'Workflows',
      subtitle: 'Automação de processos e tarefas.',
      new_workflow: 'Novo Workflow',
      trigger: 'Gatilho',
      steps: 'Passos',
      run_manual: 'Executar Manualmente',
      config_workflow: 'Configurar Workflow',
      edit_workflow: 'Editar Workflow',
      active_status: 'Status Ativo',
      step_name: 'Nome do Passo',
      step_role: 'Função Responsável',
      add_step: 'Adicionar Passo',
      run_workflow: 'Executar Workflow',
      select_properties: 'Selecionar Propriedades',
      tasks_generated_desc:
        'Tarefas serão geradas para cada propriedade selecionada.',
    },
    renewals: {
      title: 'Renovação de Contratos',
      subtitle: 'Gerencie negociações multipartes e finalize contratos.',
      search_placeholder: 'Buscar renovação...',
      all_status: 'Todos os Status',
      critical_status: 'Crítico (≤ 30 dias)',
      warning_status: 'Atenção (31 - 60 dias)',
      safe_status: 'Seguro (> 60 dias)',
      days_left: 'faltam {days} dias',
      current_proposed: 'Atual / Proposto',
      negotiation_status: 'Status da Negociação',
      renewed: 'Renovado',
      owner: 'Proprietário',
      tenant: 'Inquilino',
      manage: 'Gerenciar',
      no_results: 'Nenhuma renovação encontrada.',
      negotiation_hub: 'Central de Negociação',
      negotiation_hub_desc:
        'Gerencie renovações de aluguel de forma eficiente.',
      confidential_note: 'Confidencial: Visível apenas para os Gerentes.',
      add_note: 'Adicionar Nota Interna',
      mark_vacating: 'Marcar como Desocupando',
      close_negotiation: 'Fechar Negociação',
      close_negotiation_desc:
        'Confirme os novos termos e anexe o contrato para finalizar. O status será "Renovado".',
      new_value: 'Novo Valor do Aluguel',
      new_start_date: 'Nova Data de Início',
      new_end_date: 'Nova Data de Fim',
      contract_upload: 'Upload do Contrato',
      fill_all_fields: 'Preencha todos os campos e anexe o contrato.',
      both_must_accept: 'Ambos devem aceitar os termos antes de fechar.',
      contract_renewed: 'Contrato renovado com sucesso.',
      pricing_owner_approval: 'Preço e Aprovação do Proprietário',
      current_value: 'Valor Atual',
      proposed_value: 'Valor Proposto',
      update_terms_notify: 'Atualizar Termos e Notificar Proprietário',
      pricing_tenant_approval: 'Preço e Aprovação do Inquilino',
      new_rent_value: 'Novo Valor do Aluguel',
      pending_owner_approval: 'Aguardando Aprovação do Proprietário',
      save_decision: 'Salvar Decisão',
      type_message: 'Digite uma mensagem...',
      empty_notes: 'Nenhuma nota ainda.',
      empty_messages: 'Nenhuma mensagem ainda.',
    },
    short_term: {
      title: 'Aluguel Temporada',
      subtitle: 'Gestão de reservas de curto prazo.',
      new_booking: 'Nova Reserva',
      guest: 'Hóspede',
      check_in: 'Check-in',
      check_out: 'Check-out',
      total: 'Total',
      platform: 'Plataforma',
      reports: 'Relatórios',
      calendar: 'Calendário',
      bookings: 'Reservas',
      communication: 'Comunicação',
      revenue: 'Receita',
      occupancy: 'Ocupação',
      auto_replies: 'Respostas Automáticas',
      send_welcome: 'Enviar Mensagem de Boas-vindas',
      send_checkout_instr: 'Enviar Instruções de Check-out',
      template_name: 'Nome do Modelo',
      delay_minutes: 'Atraso (minutos)',
      amount: 'Valor',
      direct_booking: 'Direto / Manual',
      full_name_placeholder: 'Nome Completo',
      email_placeholder: 'email@hospede.com',
    },
    common: {
      add_document: 'Adicionar Documento',
      document_name: 'Nome do Documento',
      file_url: 'URL do Arquivo',
      no_documents: 'Nenhum documento ainda.',
      personal_data: 'Dados Pessoais',
      contact_address: 'Contato e Endereço',
      documents: 'Documentos',
      rg_id: 'RG / ID',
      dob: 'Data de Nasc. / Fundação',
      nationality: 'Nacionalidade',
      marital_status: 'Estado Civil',
      profession: 'Profissão',
      monthly_income: 'Renda Mensal',
      base_rent: 'Aluguel Base',
      secondary_phone: 'Telefone Secundário',
      whatsapp: 'WhatsApp',
      street: 'Logradouro',
      number: 'Número',
      complement: 'Número ou Complemento',
      neighborhood: 'Bairro',
      city: 'Cidade',
      state: 'Estado',
      zip_code: 'CEP / Zip Code',
      single: 'Solteiro(a)',
      married: 'Casado(a)',
      divorced: 'Divorciado(a)',
      widowed: 'Viúvo(a)',
      country_us: 'Estados Unidos',
      country_br: 'Brasil',
      country_es: 'Espanha',
      tax_id_label: 'CPF / CNPJ / Tax ID',
      full_name_label: 'Nome Completo / Razão Social',
      add: 'Adicionar',
      dashboard: 'Painel Imobiliário',
      real_estate_dashboard: 'Painel Imobiliário',
      properties: 'Propriedades',
      tenants: 'Inquilinos',
      owners: 'Proprietários',
      partners: 'Parceiros',
      calendar: 'Calendário',
      tasks: 'Tarefas',
      financial: 'Financeiro',
      financial_hoa: 'Financeiro HOA',
      invoices: 'Faturas',
      messages: 'Mensagens',
      settings: 'Configurações',
      service_pricing: 'Catálogo de Preços',
      search: 'Buscar...',
      filter: 'Filtrar',
      status: 'Status',
      actions: 'Ações',
      save: 'Salvar',
      cancel: 'Cancelar',
      edit: 'Editar',
      delete: 'Excluir',
      new: 'Novo',
      back: 'Voltar',
      view: 'Visualizar',
      details: 'Detalhes',
      name: 'Nome',
      email: 'Email',
      phone: 'Telefone',
      address: 'Endereço',
      type: 'Tipo',
      role: 'Cargo',
      role_label: 'Função',
      logout: 'Sair',
      profile: 'Perfil',
      notifications: 'Notificações',
      team: 'Equipe',
      automation: 'Automação',
      all: 'Todos',
      pending: 'Pendente',
      accepted: 'Aceito',
      rejected: 'Rejeitado',
      counter: 'Contraproposta',
      approved: 'Aprovado',
      completed: 'Concluído',
      paid: 'Pago',
      overdue: 'Atrasado',
      active: 'Ativo',
      inactive: 'Inativo',
      occupied: 'Ocupado',
      vacant: 'Vago',
      maintenance: 'Manutenção',
      cleaning: 'Limpeza',
      confirm: 'Confirmar',
      upload: 'Upload',
      description: 'Descrição',
      date: 'Data',
      end_date: 'Data Fim',
      due_date: 'Vencimento',
      value: 'Valor',
      priority: 'Prioridade',
      yes: 'Sim',
      no: 'Não',
      none: 'Nenhum',
      other: 'Outro',
      error: 'Erro',
      condominiums: 'Condomínios',
      continue: 'Continuar',
      password: 'Senha',
      confirm_password: 'Confirmar Senha',
      add_title: 'Adicionar',
      contact_via_whatsapp: 'WhatsApp',
      contact_via_email: 'Email',
      tax_id: 'CPF/CNPJ / Tax ID',
      system_activity: 'Atividade do Sistema',
      approve: 'Aprovar',
      reject: 'Rejeitar',
      block: 'Bloquear',
      pending_approval: 'Aguardando Aprovação',
      pending_activation: 'Pendente Ativação',
      blocked: 'Bloqueado',
      country: 'País',
      select_country: 'Selecione o País',
      export_data: 'Exportar Dados',
      export_success: 'Dados exportados com sucesso.',
      export_success_title: 'Exportação Iniciada',
      renewals: 'Renovação de Contrato',
      short_term: 'Aluguel Temporada',
      market_analysis: 'Análise de Mercado',
      advanced_analytics: 'Análise Avançada',
      automation_rules: 'Regras de Automação',
      workflows: 'Fluxos de Trabalho',
      portal: 'Portal',
      publicity: 'Publicidade',
      login: 'Entrar',
      register: 'Cadastrar',
      welcome: 'Bem-vindo',
      full_name: 'Nome Completo',
      property: 'Propriedade',
      reports: 'Relatórios',
      delete_title: 'Tem certeza?',
      delete_desc: 'Esta ação não pode ser desfeita.',
      start_now: 'Começar Agora',
      download: 'Baixar',
      print: 'Imprimir',
      close: 'Fechar',
      delete_active_tenant_error:
        'Não é possível excluir propriedade com inquilino ativo.',
      confirm_delete: 'Confirmar Exclusão',
      operations: 'Operações',
      contracts: 'Contratos',
      total: 'Total',
      done: 'Concluir',
      taxes: 'Impostos',
      utilities: 'Utilidades',
      inventory: 'Inventário',
      history: 'Histórico',
      activity_log: 'Log de Atividade',
      created_at: 'Criado em',
      updated_at: 'Atualizado em',
      select: 'Selecione',
      required: 'Obrigatório',
      success: 'Sucesso',
      removed: 'Removido',
      copied: 'Copiado',
      empty: 'Vazio',
      relationships: 'Relacionamentos',
      relationship_owner: 'Proprietário',
      relationship_tenant: 'Inquilino',
      no_owner: 'Sem Proprietário',
      add_owner: 'Adicionar Proprietário',
      no_active_tenant: 'Sem Inquilino Ativo',
      export: 'Exportar',
      visits: 'Visitas',
      schedule_visit: 'Agendar Visita',
      schedule_maintenance: 'Agendar Manutenção',
      client_name: 'Nome do Cliente',
      loading: 'Carregando...',
      unknown: 'Desconhecido',
      resource: 'Recurso',
      profile: 'Perfil',
      visit_date: 'Data da Visita',
      visit_status: 'Status da Visita',
      scheduled: 'Agendada',
      canceled: 'Cancelada',
      visit_scheduling: 'Agendamento de Visitas',
      visits_list: 'Lista de Visitas',
      total_revenue: 'Receita Total',
      sold: 'Vendido',
      sale_pending: 'Venda Pendente',
      category: 'Categoria',
      quantity: 'Quantidade',
      condition: 'Condição',
      revenue: 'Receita',
      profit: 'Lucro',
      provider: 'Provedor',
      analytics: {
        benchmark_title: 'Benchmark de Mercado',
        benchmark_desc: 'Comparativo de desempenho com o mercado.',
        house_model: 'Modelo da Casa',
        internal_perf: 'Performance Interna',
        market_avg: 'Média de Mercado',
        occupancy: 'Ocupação',
        profitability_title: 'Rentabilidade',
        profit: 'Lucro',
        revenue: 'Receita',
        adr: 'Diária Média (ADR)',
        privacy_mode: 'Modo de Privacidade',
      },
      help_hub: 'Central de Ajuda',
      hotels: 'Hotéis',
      location: 'Localização',
      access_credentials: 'Credenciais de Acesso',
      manager: 'Gerente',
      contact: 'Contato',
      history_adjustments: 'Histórico de Ajustes',
      name_required: 'Nome é obrigatório',
      email_invalid: 'Email inválido',
      error_404_title: '404',
      error_404_desc: 'Ops! Página não encontrada.',
      return_home: 'Voltar para o Início',
      access_denied: 'Acesso Negado',
      access_denied_desc: 'Você não tem permissão para visualizar esta página.',
      list: 'Lista',
      calendar_view: 'Calendário',
      start_date: 'Data Início',
      and: 'e',
      validation_error: 'Erro de Validação',
      delete_success: 'Excluído com sucesso.',
      error_delete: 'Erro ao excluir.',
      delete_linked_error:
        'Não é possível excluir pois existem registros vinculados.',
      new_task: 'Nova Tarefa',
      create_title: 'Criar Tarefa',
      search_address: 'Buscar Endereço',
    },
    dashboard: {
      title: 'Painel de Controle Central',
      desc_main:
        'Visão gerencial completa: Performance, Finanças, Hospitalidade e Operações.',
      integrated_management: 'Gestão Integrada',
      tabs: {
        overview: 'Visão Geral',
        financial: 'Financeiro',
        hospitality: 'Hospitalidade',
        operations: 'Operações & Equipe',
      },
      cards: {
        total_revenue: 'Receita Total (Mês)',
        occupancy_rate: 'Taxa de Ocupação',
        default_rate: 'Inadimplência',
        portfolio: 'Portfólio',
        vs_last_month: 'em relação ao mês anterior',
        occupied_properties: 'imóveis ocupados',
        pending_amount: 'pendente',
        active_properties: 'Propriedades sob gestão ativa',
      },
      charts: {
        cash_flow: 'Fluxo de Caixa Consolidado',
        cash_flow_desc: 'Receitas e Despesas ao longo do tempo',
        portfolio_status: 'Status do Portfólio',
        portfolio_status_desc: 'Distribuição de ocupação',
        revenue: 'Receita',
        expense: 'Despesa',
        net_profit: 'Lucro Líquido',
      },
      financial_tab: {
        noi: 'Lucro Operacional Líquido (NOI)',
        this_month: 'este mês',
        total_expenses: 'Despesas Totais',
        above_target: '(Acima da meta)',
        profit_margin: 'Margem de Lucro',
        industry_avg: 'Média saudável da indústria: 50-60%',
        revenue_growth: 'Crescimento de Receita (Evolução Contínua)',
        gross_revenue: 'Receita Bruta',
      },
      hospitality_tab: {
        active_hotels: 'Hotéis Ativos',
        bookings_today: 'Reservas (Hoje)',
        adr: 'ADR (Diária Média)',
        revpar: 'RevPAR',
        metrics_6m: 'Métricas de Hospitalidade (Últimos 6 meses)',
        occupancy_pct: 'Ocupação (%)',
      },
      operations_tab: {
        pending_tasks: 'Tarefas Pendentes',
        completed_tasks: 'Tarefas Concluídas',
        active_tenants: 'Inquilinos Ativos',
        pending_renewals: 'Renovações Pendentes',
        tenant_profile: 'Perfil de Inquilinos',
        latest_issues: 'Últimas Ocorrências',
        maintenance_ac: 'Manutenção Ar Condicionado',
        unit: 'Unidade',
      },
      demographics: {
        long_term: 'Longo Prazo',
        short_term: 'Curto Prazo',
        commercial: 'Comercial',
      },
      status: {
        rented: 'Alugado',
        available: 'Disponível',
        maintenance: 'Manutenção',
      },
      months: [
        'Jan',
        'Fev',
        'Mar',
        'Abr',
        'Mai',
        'Jun',
        'Jul',
        'Ago',
        'Set',
        'Out',
        'Nov',
        'Dez',
      ],
      unread: 'não lidas',
      from_last_month: 'do último mês',
      customize: 'Personalizar',
      customize_view: 'Personalizar Visualização',
      customize_desc: 'Selecione os widgets que deseja ver.',
      kpi_indicators: 'Indicadores KPI',
      revenue_chart: 'Gráfico de Receita',
      revenue_vs_expenses: 'Receita vs Despesas',
      revenue_overview: 'Visão geral financeira dos últimos meses.',
      quick_calendar: 'Calendário Rápido',
      todays_activity: 'Atividades de hoje.',
      pending_approvals: 'Aprovações Pendentes',
      pending_desc: 'Itens que requerem sua atenção.',
      review: 'Revisar',
      approve: 'Aprovar',
      expense_distribution: 'Distribuição de Despesas',
      expense_desc: 'Detalhamento de gastos por categoria.',
      subtitle: 'Bem-vindo de volta! Aqui está o resumo das suas operações.',
    },
    gamification: {
      health_score: 'Health Score',
      portfolio_health: 'Saúde do Portfólio',
      desc: 'Pontuação baseada em ocupação, manutenção e avaliações.',
      level: 'Nível',
      expert: 'Expert',
      target: 'Meta',
    },
    properties: {
      title: 'Propriedades',
      subtitle: 'Gerencie seu portfólio imobiliário.',
      new_property: 'Nova Propriedade',
      add_title: 'Adicionar Propriedade',
      add_description: 'Preencha o formulário para criar uma nova propriedade.',
      rental_type: 'Tipo de Aluguel',
      profile_short: 'Curto Prazo',
      profile_long: 'Longo Prazo',
      search_placeholder: 'Buscar propriedade...',
      address_placeholder: 'Endereço da propriedade',
      zip_code: 'CEP',
      zip_required: 'CEP é obrigatório',
      info_label: 'Info Adicional',
      city_placeholder: 'Cidade',
      state_placeholder: 'Estado',
      property_value: 'Valor da Propriedade',
      hoa_fee: 'Taxa de Condomínio',
      hoa_auto_hint: 'Cálculo automático mensal',
      cover_image: 'Imagem de Capa',
      no_image_selected: 'Nenhuma imagem selecionada',
      no_image: 'Sem Imagem',
      view_details: 'Ver Detalhes',
      property_added: 'Propriedade Adicionada',
      validation_error: 'Erro de Validação',
      name_required: 'Nome/Apelido é obrigatório',
      address_required: 'Endereço é obrigatório',
      condominium_link: 'Vínculo com Condomínio',
      select_condominium: 'Selecionar Condomínio',
      condo_inherited_info: 'Dados de acesso e contato serão herdados.',
      property_nickname: 'Nome / Apelido',
      nickname_placeholder: 'Ex: Casa de Praia, Villa Sol',
      photos: 'Fotos (Selecione a Capa)',
      cover: 'Capa',
      city_state_required: 'Cidade e Estado são obrigatórios',
      profile_required: 'Tipo de Perfil é obrigatório',
      delete_success: 'Propriedade excluída com sucesso.',
      error_delete: 'Erro ao excluir propriedade.',
      all_profiles: 'Todos Perfis',
      profile_filter: 'Filtro de Perfil',
      monthly: 'Mensal',
      quarterly: 'Trimestral',
      annually: 'Anual',
      hoa_freq: 'Frequência HOA',
      overview: 'Visão Geral',
      house: 'Casa',
      condo: 'Condomínio',
      townhouse: 'Sobrado',
      marketing: 'Marketing',
      publish_portals: 'Publicar nos Portais',
      upload_image: 'Carregar Imagem',
      independent_community: 'Independente',
      additional_info_placeholder: 'Apto, Bloco, etc.',
      tabs: {
        overview: 'Visão Geral',
        analytics: 'Análises',
        reports: 'Relatórios',
        maintenance: 'Manutenção',
        contracts: 'Contratos',
        inventory: 'Inventário',
        features: 'Características',
        location: 'Localização',
        sync: 'Sincronização',
        financial: 'Financeiro',
        marketing: 'Marketing',
        content: 'Conteúdo',
        documents: 'Documentos',
        logs: 'Logs',
        pricing: 'Preços',
      },
      location: {
        address: 'Endereço',
        neighborhood: 'Bairro',
        community: 'Condomínio / Comunidade',
        linked_condo: 'Condomínio Vinculado',
        complement_placeholder: 'Apto, Bloco, Referência',
        country_us: 'Estados Unidos (EUA)',
        country_br: 'Brasil',
        country_es: 'Espanha',
      },
      contacts: {
        new_contact: 'Novo Contato',
        role_concierge: 'Concierge',
        role_maintenance: 'Manutenção',
        role_emergency: 'Emergência',
        role_local_agent: 'Agente Local',
      },
      content: {
        house_rules: 'Regras do Imóvel',
        auto_translate: 'Tradução Automática',
        copy_from: 'Copiar de',
        desc_pt_placeholder: 'Descrição completa em Português...',
        desc_en_placeholder: 'Descrição completa em Inglês...',
        desc_es_placeholder: 'Descrição completa em Espanhol...',
        rules_pt_placeholder: 'Regras do condomínio em Português...',
        rules_en_placeholder: 'Regras do condomínio em Inglês...',
        rules_es_placeholder: 'Regras do condomínio em Espanhol...',
      },
      financial: 'Financeiro',
      financial_fields: {
        name_placeholder: 'Ex: Internet',
        provider_placeholder: 'Ex: Duke Energy',
        account_placeholder: 'ID da Conta',
      },
      features: {
        bedrooms: 'Quartos',
        bathrooms: 'Banheiros',
        guests: 'Hóspedes',
        wifi_connectivity: 'Conectividade Wi-Fi',
        ssid: 'SSID / Nome da Rede',
        password: 'Senha',
        access_codes: 'Códigos de Acesso',
        unit_code: 'Código da Unidade',
        building_code: 'Código do Edifício',
        pool_code: 'Código da Piscina',
        staff_code: 'Código do Staff / Limpeza',
        condo_read_only: 'Acesso do Condomínio (Somente Leitura)',
        condo_hint:
          'Credenciais gerenciadas nas configurações do condomínio {condo}.',
      },
    },
    auth: {
      login_title: 'Login',
      login_desc: 'Entre na sua conta para continuar',
      email_placeholder: 'exemplo@empresa.com',
      forgot_password: 'Esqueceu a senha?',
      no_account: 'Não tem uma conta?',
      demo_login: 'Login Demo',
      register_title: 'Criar Conta',
      register_desc: 'Comece a gerenciar suas propriedades hoje',
      name_placeholder: 'Seu nome completo',
      company_details: 'Detalhes da Empresa',
      company_placeholder: 'Nome da sua empresa',
      tax_id_placeholder: 'CNPJ / Tax ID',
      address_placeholder: 'Endereço comercial',
      already_have_account: 'Já tem uma conta?',
    },
    settings: {
      title: 'Configurações',
      subtitle: 'Gerencie as configurações da sua conta e sistema.',
      personal_info: 'Informações Pessoais',
      update_info: 'Atualize suas informações.',
      change_photo: 'Alterar Foto',
      full_name: 'Nome Completo',
      save_changes: 'Salvar Alterações',
      integrations: 'Integrações',
      billing_payment: 'Faturamento e Pagamento',
      billing_gateways_title: 'Gateways de Pagamento',
      billing_gateways_desc: 'Configure os métodos de pagamento aceitos.',
      payment_gateways: 'Gateways de Pagamento',
      system_alerts: 'Alertas do Sistema',
      system_alerts_desc: 'Configure as notificações automáticas.',
      automation_title: 'Regras de Automação',
      automation_desc:
        'Gerencie e configure regras de automação para tarefas, finanças e comunicação.',
      rent_reminder: 'Lembrete de Aluguel',
      days_before: 'Dias Antes',
      company_legal_name: 'Razão Social',
    },
    status: {
      rented: 'Alugado',
      available: 'Disponível',
      sold: 'Vendido',
      occupied: 'Ocupado',
      vacant: 'Vago',
      suspended: 'Suspenso',
      maintenance: 'Manutenção',
      reserved: 'Reservado',
      rescheduled: 'Reagendado',
      released: 'Liberado',
      in_registration: 'Em Cadastro',
      interested: 'Interessado',
      negotiating: 'Negociando',
      owner_contacted: 'Proprietário Contactado',
      tenant_contacted: 'Inquilino Contactado',
      vacating: 'Desocupando',
    },
    roles: {
      platform_owner: 'Admin',
      software_tenant: 'Gerente',
      internal_user: 'Staff',
      partner: 'Parceiro',
      property_owner: 'Proprietário',
      tenant: 'Inquilino',
      partner_employee: 'Funcionário Parceiro',
    },
    marketing_tab: {
      portal_sync: 'Sincronização de Portais',
      manage_listings: 'Gerenciar anúncios em portais externos.',
      publish_btn: 'Publicar',
      publishing: 'Publicando...',
      publish_success: 'Publicação iniciada com sucesso.',
      leads: 'Leads e Interessados',
      inquiries_desc: 'Gerencie consultas de potenciais inquilinos e hóspedes.',
      contact_info: 'Informações de Contato',
      source: 'Origem',
      inquiry_date: 'Data da Consulta',
      status: 'Status',
      zillow_desc: 'Líder do Mercado EUA',
      idealista_desc: 'Mercado Europeu',
    },
    owner_portal: {
      welcome: 'Bem-vindo',
      subtitle: 'Portal de Ativos do Proprietário',
      registered_properties: 'Propriedades Registradas',
      pending_tasks: 'Aprovações de Tarefas Pendentes',
      contact_pm: 'Falar com a PM',
      pending_hoa: 'Condomínio e Impostos',
      pending_approvals: 'Aprovações de Custos',
      pending_renewals: 'Renovações Pendentes',
      ledger: 'Conta Corrente & Financeiro',
      hoa_taxes: 'Condomínio e Impostos',
      damages_costs: 'Danos & Custos',
      renewals: 'Renovações',
      properties: 'Propriedades',
      hoa_desc: 'Gerencie suas taxas de condomínio e impostos.',
      go_to_financial: 'Ir para o Financeiro',
      no_records: 'Nenhum registro encontrado',
      no_hoa_records:
        'Nenhuma taxa de condomínio ou imposto registrada para suas propriedades.',
      sandbox_tasks: 'Tarefas de Sandbox (Apenas Desenvolvimento)',
      long_term_renewals: 'Renovações de Contrato de Longo Prazo',
      no_pending_renewals: 'Nenhuma renovação pendente',
      no_pending_renewals_desc:
        'Não há contratos de longo prazo aguardando sua aprovação neste momento.',
      current_rent: 'Aluguel Atual',
      proposed_rent: 'Aluguel Proposto',
      cost_approvals: 'Aprovações de Custos (Acima do Limite)',
      estimated_cost: 'Custo Estimado',
      suggested_date: 'Data Sugerida',
      maintenance_history: 'Histórico de Danos e Custos de Manutenção',
      no_tasks:
        'Nenhuma tarefa registrada para as propriedades deste proprietário no período selecionado.',
      task_approved: 'Tarefa Aprovada',
      task_rejected: 'Tarefa Recusada',
      task_status_updated: 'O status da tarefa foi atualizado com sucesso.',
    },
    automation: {
      title: 'Automação',
      quickbooks_export: 'Exportar para QuickBooks',
      quickbooks_desc: 'Baixe transações compatíveis com QB.',
      export_csv: 'Exportar CSV',
      export_success_title: 'Exportação Iniciada',
      auto_generate_invoice: 'Gerar Fatura Automática',
      generate_invoice: 'Gerar Fatura',
      export_csv: 'Exportar (CSV)',
      auto_approval: 'Auto-Aprovação',
      auto_approval_desc:
        'Aprovar automaticamente tarefas de manutenção abaixo de um determinado custo.',
      enable_auto_approval: 'Ativar Auto-Aprovação',
      billing_automation: 'Automação de Faturamento',
      billing_automation_desc:
        'Gerar faturas automaticamente com base em eventos.',
      auto_generate_invoices: 'Gerar Faturas Automaticamente',
      rent_reminders_desc: 'Enviar lembretes automáticos para inquilinos.',
      enable_reminders: 'Ativar Lembretes',
      rule_enabled: 'Regra habilitada',
      max_cost_threshold: 'Limite Máximo de Custo',
      tasks_below_threshold:
        'Tarefas abaixo deste valor serão aprovadas instantaneamente.',
      trigger_event: 'Evento Gatilho',
      confirm_export: 'Confirmar Exportação',
      triggering: 'Disparando',
      action: 'Ação',
      status: 'Status da Automação',
      trigger_desc:
        'Faturas serão geradas quando o status da tarefa mudar para "Concluído".',
      auto_sms_desc: 'Email/SMS automático será enviado.',
      select_export_scope: 'Selecione o Escopo de Exportação',
      choose_data_range:
        'Escolha o intervalo de dados para a exportação QuickBooks.',
      data_scope: 'Escopo de Dados',
      general_activities: 'Atividades Gerais',
      by_owner: 'Por Proprietário',
      by_partner: 'Por Parceiro',
      task_automation: 'Automação de Tarefas',
      financial_automation: 'Automação Financeira',
      communication_automation: 'Automação de Comunicação',
      quickbooks_integration: 'Integração QuickBooks',
    },
  },
  en: {
    sync: {
      title: 'Basic iCal Synchronization',
      sync_desc: 'Simple calendar syncing via URL.',
      platform: 'Platform',
      ical_url: 'iCal URL',
      add_link: 'Add Link',
      status: 'Status',
      last_sync: 'Last Sync',
      no_links: 'No links configured.',
      sync_now: 'Sync Now',
      sync_success: 'Sync Started',
      export_calendar: 'Export Calendar',
      copy_link: 'Copy Link',
      airbnb: 'Airbnb (iCal)',
      booking: 'Booking.com (iCal)',
      vrbo: 'Vrbo (iCal)',
    },
    publicity: {
      title: 'Publicity Administration',
      subtitle: 'Manage advertisers, campaigns, and monetization settings.',
      tab_ads: 'Advertisements',
      tab_advertisers: 'Advertisers',
      tab_pricing: 'Pricing Config',
      advertiser_list: {
        title: 'Advertisers',
        search_placeholder: 'Search advertisers...',
        add_btn: 'Add Advertiser',
        table_company: 'Company / Name',
        table_contact: 'Contact',
        table_address: 'Address',
        table_registered: 'Registered',
        empty_state: 'No advertisers found.',
        modal_edit: 'Edit Advertiser',
        modal_new: 'New Advertiser',
        label_company: 'Company / Name *',
        label_email: 'Email *',
        label_phone: 'Phone',
        label_address: 'Address',
        delete_confirm: 'Are you sure you want to delete this advertiser?',
        delete_success: 'Advertiser deleted.',
        update_success: 'Advertiser updated successfully.',
        add_success: 'Advertiser added successfully.',
        validation_error: 'Name and email are required.',
      },
      ads_manager: {
        title: 'Advertisements Manager',
        search_placeholder: 'Search ads...',
        add_btn: 'New Advertisement',
        table_ad_info: 'Ad Info',
        table_advertiser: 'Advertiser',
        table_placement: 'Placement',
        table_validity: 'Validity',
        table_price: 'Price',
        table_status: 'Status',
        empty_state: 'No advertisements found.',
        modal_edit: 'Edit Advertisement',
        modal_new: 'New Advertisement',
        label_title: 'Title *',
        label_advertiser: 'Advertiser *',
        select_advertiser: 'Select advertiser',
        label_placement: 'Placement Location',
        label_price: 'Price',
        label_start_date: 'Start Date',
        label_end_date: 'End Date',
        label_image_url: 'Image URL *',
        label_target_link: 'Target Link URL',
        active_status: 'Active Status',
        active_status_desc: 'Toggle ad visibility across the platform.',
        save_ad: 'Save Advertisement',
        delete_confirm: 'Are you sure you want to delete this ad?',
        delete_success: 'Advertisement deleted.',
        update_success: 'Advertisement updated successfully.',
        add_success: 'Advertisement created successfully.',
        validation_error: 'Title, Image URL, and Advertiser are required.',
        link: 'Link',
        active: 'Active',
        inactive: 'Inactive',
        unknown: 'Unknown',
        placements: {
          home_top: 'Home Top Banner',
          home_bottom: 'Home Bottom Banner',
          partner_page: 'Partner Page',
          tenant_page: 'Tenant Portal',
          pm_login: 'PM Login Screen',
          sidebar: 'Sidebar',
          footer: 'Footer',
        },
      },
      pricing_config: {
        title_base: 'Base Duration Rates',
        desc_base:
          'Set the standard pricing for different advertisement durations.',
        label_weekly: 'Weekly Rate',
        label_biweekly: 'Bi-Weekly Rate (14 Days)',
        label_monthly: 'Monthly Rate',
        title_modifiers: 'Placement Modifiers',
        desc_modifiers: 'Additional costs for premium placements.',
        tooltip_modifiers:
          'These values represent fixed additions to the base duration rate depending on where the ad is placed.',
        save_btn: 'Save Configuration',
        save_success_title: 'Pricing Configuration Saved',
        save_success_desc: 'Your ad rates have been updated successfully.',
      },
    },
    sidebar: {
      dashboard: 'Dashboard',
      portfolio: 'Portfolio',
      units: 'Properties',
      condominiums: 'Condominiums',
      owners: 'Owners',
      tenants: 'Tenants',
      financial: 'Financial',
      marketing: 'Marketing',
      reports: 'Reports',
      settings: 'Settings',
      performance: 'Performance',
      guest_services: 'Guest Services',
      pos: 'Point of Sale',
      main_menu: 'Main Menu',
      system: 'System',
      migration_hub: 'Migration',
      publicity_admin: 'Publicity',
      users: 'Users',
      tenant_portal: 'Tenant Portal',
      owner_portal: 'Owner Portal',
      partner_portal: 'Partner Portal',
      housekeeping: 'Housekeeping',
      night_audit: 'Night Audit',
      calendar: 'Calendar',
      partners: 'Partners',
    },
    housekeeping: {
      title: 'Housekeeping',
      subtitle: 'Manage cleaning tasks and schedules.',
      task_title: 'Task Title',
      property: 'Property',
      assignee: 'Assignee',
      date: 'Date',
      status: 'Status',
    },
    hotel_form: {
      hotel_name: 'Hotel Name',
      street_address: 'Street Address',
      number: 'Number',
      neighborhood: 'Neighborhood',
      city: 'City',
      state: 'State / Province',
      zip_code: 'Zip Code',
      towers_wings: 'Towers / Wings',
      add_division: 'Add Division',
      define_divisions: 'Define internal divisions like Towers or Wings.',
      validation_error: 'Name and City are required.',
      placeholder_name: 'E.g. Grand Heritage Hotel',
      details: 'Details',
      location: 'Location',
      no_divisions: 'No divisions added. Click "Add Division" to create one.',
      delete_title: 'Delete Hotel',
      delete_desc:
        'This action cannot be undone. Are you sure you want to permanently delete this hotel?',
    },
    hotels: {
      title: 'Hotels',
      subtitle: 'Manage hotels and rooms.',
      add_title: 'Add Hotel',
      new_hotel: 'New Hotel',
      towers: 'Towers',
      rooms: 'Rooms',
      floors: 'Floors',
      add_tower: 'Add Tower',
      new_tower: 'New Tower',
      hotel_details: 'Hotel Details',
      no_hotels: 'No hotels found.',
      not_found: 'Hotel Not Found',
      update_success: 'Hotel updated successfully.',
      delete_success: 'Hotel deleted.',
      tower_add_success: 'Tower added successfully.',
      all_rooms: 'All Rooms',
      info: 'Hotel Information',
      towers_wings: 'Towers & Wings',
      add_new_tower: 'Add New Tower/Wing',
      tower_name: 'Tower Name',
      manage_rooms: 'Manage Rooms',
      no_towers: 'No towers or wings defined.',
    },
    header: {
      demo_profiles: 'Demo Profiles',
      other_users: 'Other Users',
    },
    market_analysis: {
      title: 'Market Analysis',
      subtitle: 'Market benchmark and trends.',
      demand_index: 'Demand Index',
      peak_season: 'Peak season approaching',
      comp_set_rank: 'Comp Set Rank',
      rank_desc: 'Out of 12 local hotels',
      competitor_pricing: 'Competitor Pricing',
      demand_forecast: 'Demand Forecast',
      no_forecast: 'No forecast data available.',
      avg_occupancy: 'Avg Occupancy',
      avg_daily_rate: 'Avg Daily Rate (ADR)',
      spending_trends: 'Spending Trends',
      high: 'High',
      medium: 'Medium',
      low: 'Low',
      historical_view: 'Historical view of market performance',
      avg_rate_comparison: 'Average daily rate comparison',
      projected_demand: 'Projected demand levels for upcoming dates',
    },
    pos: {
      title: 'Point of Sale',
      subtitle: 'Manage products, sales, and stock.',
      new_product: 'New Product',
      manage_products: 'Manage Products',
      product_name: 'Product Name',
      price: 'Price',
      category: 'Category',
      validity_start: 'Valid From',
      active: 'Active',
      inactive: 'Inactive',
      stock: 'Stock',
      actions: 'Actions',
      create: 'Create',
      edit: 'Edit',
      inactivate: 'Inactivate',
      delete: 'Delete',
      confirm_delete: 'Confirm Delete',
      delete_desc: 'Are you sure you want to delete this product?',
      save_success: 'Product saved successfully.',
      add_to_cart: 'Add to Cart',
      checkout: 'Checkout',
      charge_room: 'Charge Room',
      out_of_stock: 'Out of Stock',
      cart_empty: 'Cart Empty',
      transaction_success: 'Transaction successful!',
      laundry_service: 'Laundry Service',
      laundry: 'Laundry',
      club_sandwich: 'Club Sandwich',
      restaurant: 'Restaurant',
      minibar: 'Minibar',
      cola: 'Cola',
      chips: 'Chips',
      guest: 'Guest',
      select: 'Select',
    },
    guest_services: {
      title: 'Guest Services',
      subtitle: 'Service catalog and scheduling.',
      new_service: 'New Service',
      manage_services: 'Manage Services',
      financial_insights: 'Financial Insights',
      service_name: 'Service Name',
      description: 'Description',
      price: 'Price',
      category: 'Category',
      validity_start: 'Valid From',
      seasonal_pricing: 'Seasonal Pricing',
      add_seasonal_price: 'Add Seasonal Price',
      start_date: 'Start Date',
      end_date: 'End Date',
      schedule: 'Schedule',
      schedule_for: 'Schedule For',
      assign_service: 'Assign Service',
      service_assigned: 'Service assigned successfully.',
      save_success: 'Service saved successfully.',
    },
    performance: {
      guest_reviews: 'Guest Reviews',
      average_rating: 'Average Rating',
      based_on_reviews: 'Based on {count} reviews',
      response: 'Response',
      no_reviews: 'No reviews yet.',
      rating_reviews_trend: 'Rating & Reviews Trend',
      guest_satisfaction_evolution:
        'Evolution of guest satisfaction over time.',
      financial_performance: 'Financial Performance (RevPAR)',
      revpar_trends: 'Revenue per Available Room trends.',
    },
    marketing: {
      promotions: 'Promotions',
      campaigns: 'Campaigns',
      automation: 'Automation',
      leads: 'Leads & Prospects',
      new_code: 'New Code',
      create_promo: 'Create Promo Code',
      discount: 'Discount',
      validity: 'Validity',
      usage: 'Usage',
      new_campaign: 'New Campaign',
      create_campaign: 'Create Campaign',
      target_audience: 'Target Audience',
      code: 'Code',
      automated_workflows: 'Automated Workflows',
      automated_desc: 'Trigger emails based on events.',
      email_templates: 'Email Templates',
      email_desc: 'Reusable content with placeholders.',
      trigger_event: 'Trigger Event',
      timing: 'Timing',
      template: 'Template',
      subject: 'Subject',
      body: 'Body',
      placeholders: 'Available variables',
      active: 'Active',
      booking_confirmed: 'Booking Confirmed',
      check_in: 'Check-in',
      check_out: 'Check-out',
      cancellation: 'Cancellation',
      immediately: 'Immediately',
      hours: 'hours',
      before: 'before',
      after: 'after',
      new_message: 'New Message',
      new_workflow: 'New Workflow',
    },
    analytics_page: {
      occupancy_comparison: 'Occupancy Comparison',
      profitability_comparison: 'Profitability Comparison',
      internal_performance: 'Internal Performance',
      market_average: 'Market Average',
      benchmark_desc: 'Performance comparison with market data.',
    },
    reports: {
      title: 'Advanced Reports',
      subtitle: 'Inventory, maintenance, and property condition analysis.',
      period: 'Period',
      property: 'Property',
      apply_filters: 'Apply Filters',
      maintenance_analytics: 'Maintenance Analytics',
      inventory_health: 'Inventory Health',
      maintenance_efficiency: 'Maintenance Efficiency',
      inventory_condition: 'Inventory Condition',
      damage_by_property: 'Damage by Property',
      total_items: 'Total Items',
      damaged_poor: 'Damaged/Poor',
      total_spend: 'Total Spend',
      total_tasks: 'Total Tasks',
      avg_resolution: 'Avg Resolution',
      avg_cost_task: 'Avg Cost/Task',
      cost_breakdown: 'Cost Breakdown',
      distribution_expenses: 'Expense Distribution',
      monthly_expenditure: 'Monthly Expenditure',
      spending_trends: 'Spending Trends',
      reports: 'Reports',
    },
    service_pricing: {
      title: 'Price Catalog',
      desc: 'Price catalog for generic services and products.',
      subtitle: 'Manage costs and sales prices.',
      catalog_tab: 'Price Catalog',
      analytics_tab: 'Reports',
      new_rate: 'New Rate',
      service_name: 'Service Name',
      partner_payment: 'Partner Payment',
      pm_value: 'PM Value',
      product_price: 'Sale Price',
      cost_price: 'Cost Price',
      valid_from: 'Valid From',
      valid_to: 'Valid To',
      margin: 'Margin',
      distribution: 'Distribution',
      profit_analysis: 'Profit Analysis',
    },
    condominiums: {
      title: 'Condominiums',
      subtitle: 'Manage condominiums and HOAs.',
      new_condo: 'New Condo',
      add_title: 'Add Condo',
      manager: 'Manager',
      contact: 'Contact',
      no_condos: 'No condominiums found.',
      general_info: 'General Info',
      addressing: 'Addressing',
      access_codes: 'Access Codes',
      management: 'Management',
      description: 'Description',
      street: 'Street',
      number: 'Number',
      complement: 'Number or complement',
      neighborhood: 'Neighborhood',
      city: 'City',
      state: 'State',
      zip_code: 'Zip Code',
      manager_name: 'Manager Name',
      manager_phone: 'Manager Phone',
      manager_email: 'Manager Email',
      gate_code: 'Gate Code',
      pedestrian_gate: 'Pedestrian Gate',
      pool_code: 'Pool Code',
      amenities_code: 'Amenities Code',
      edit_condo: 'Edit Condo',
      view_details: 'View Details',
      search_placeholder: 'Search condominium...',
      delete_confirm: 'Are you sure you want to delete this condominium?',
    },
    owners: {
      title: 'Owners',
      subtitle: 'Owner and investor database.',
      new_owner: 'New Owner',
      register_title: 'Register Owner',
      base_title: 'Owner Database',
      search_placeholder: 'Search owner...',
      contact_details: 'Contact Details',
      properties_count: 'Properties',
      workflow_started: 'Workflow Started',
      workflow_desc: '{action} action started for {name}.',
      renew_contract: 'Renew Contract',
      success_desc: 'Owner registered successfully.',
      add_owner: 'Add Owner',
      edit_owner: 'Edit Owner',
      owner_added: 'Owner added successfully',
      owner_updated: 'Owner updated successfully',
      delete_owner: 'Delete Owner',
      delete_owner_desc: 'This action cannot be undone.',
    },
    tenants: {
      title: 'Tenants',
      subtitle: 'Contract and tenant management.',
      new_tenant: 'New Tenant',
      register_title: 'Register Tenant',
      list_title: 'Tenant List',
      search_placeholder: 'Search tenant...',
      property: 'Property',
      send_message: 'Send Message',
      add_tenant: 'Add Tenant',
      edit_tenant: 'Edit Tenant',
      tenant_added: 'Tenant added successfully',
      tenant_updated: 'Tenant updated successfully',
      delete_tenant: 'Delete Tenant',
      delete_tenant_desc: 'This action cannot be undone.',
    },
    partners: {
      title: 'Partners',
      subtitle: 'Vendors and service providers.',
      new_partner: 'New Partner',
      register_title: 'Register Partner',
      no_partners: 'No partners found.',
      search_placeholder: 'Search partner...',
      contact_name: 'Contact Name',
      company_name: 'Company Name',
      category: 'Category',
      agent: 'Agent',
      cleaning: 'Cleaning',
      maintenance: 'Maintenance',
      bank_info: 'Bank Info',
      bank_name: 'Bank Name',
      routing: 'Routing',
      account: 'Account',
      payment_history: 'Payment History',
      financial_report: 'Financial Report',
      inspection: 'Inspection',
    },
    invoices: {
      title: 'Invoices',
      subtitle: 'Billing and payment management.',
      create_new: 'Create New',
      invoice_id: 'Invoice ID',
      amount: 'Amount',
      bill_to: 'Bill To',
      status_paid: 'Paid',
      status_pending: 'Pending',
      status_approved: 'Approved',
      no_invoices: 'No invoices found.',
      send_to_payer: 'Send to Payer',
      sent_success: 'Invoice sent successfully.',
      view: 'View',
      invoice_viewer: {
        title: 'Invoice Viewer',
        invoice_no: 'Invoice No.',
        date: 'Date',
        bill_to: 'Bill To',
        description: 'Description',
        quantity: 'Qty',
        amount: 'Amount',
        subtotal: 'Subtotal',
        total: 'Total',
        thank_you: 'Thank you for your business!',
        print: 'Print',
        download: 'Download',
      },
    },
    financial: {
      title: 'Financial',
      subtitle: 'Financial overview and reconciliation.',
      management_desc: 'Current accounts, fixed and variable costs management.',
      add_transaction: 'Add Transaction',
      add_title: 'Include Transaction',
      export_csv: 'Export (CSV)',
      type: 'Type',
      date: 'Date',
      description: 'Description',
      category: 'Category',
      amount: 'Amount',
      property_optional: 'Property (Optional)',
      property: 'Property',
      cost_category: 'Cost Category',
      is_recurring: 'Is this a recurring expense/income?',
      recurrence_freq: 'Recurrence Frequency',
      recurring_hint:
        'When marking this item as "Paid", the system will automatically generate the entry for the next month/year.',
      initial_status: 'Initial Status',
      save_transaction: 'Save Transaction',
      balances_panel: 'Balances and Reports Panel',
      balances_desc:
        'View the current account balance filtered by your preference.',
      pm_general: 'PM (General)',
      owner: 'Owner',
      select_owner: 'Select Owner',
      all_owners: 'All Owners',
      all_properties: 'All Properties',
      viewing_your_properties: 'Viewing Your Properties',
      filters: 'Filters:',
      all_categories: 'All Categories',
      incomes: 'Incomes',
      expenses: 'Expenses',
      maintenances: 'Maintenances',
      cleanings: 'Cleanings',
      hoa_condo: 'HOA / Condo',
      taxes: 'Taxes',
      all_period: 'All Period',
      current_month: 'Current Month',
      semester: 'Semester',
      fiscal_year: 'Fiscal Year',
      custom: 'Custom',
      projected_balance: 'Projected Final Balance',
      projected_balance_desc:
        'Projected balance including the filtered history',
      total_incomes: 'Total Incomes (In Period)',
      total_expenses: 'Total Expenses (In Period)',
      pending_suffix: 'pending',
      statement: 'Transaction Statement',
      table_date: 'Date',
      table_desc: 'Description',
      table_type_cat: 'Type / Category',
      table_status: 'Status',
      table_amount: 'Amount',
      table_balance: 'Balance',
      table_actions: 'Actions',
      monthly: 'Monthly',
      yearly: 'Yearly',
      income: 'Income',
      expense: 'Expense',
      fixed_cost: 'Fixed Cost',
      variable_cost: 'Variable Cost',
      cleared: 'Paid/Received',
      pay: 'Pay',
      edit_transaction: 'Edit Transaction',
      save_changes: 'Save Changes',
      delete_transaction: 'Delete Transaction',
      delete_confirm:
        'Are you sure you want to delete this transaction? This action cannot be undone.',
      cancel: 'Cancel',
      delete: 'Delete',
      no_transactions: 'No transactions found for the selected filters.',
      success_add: 'Transaction added successfully',
      success_edit: 'Transaction updated successfully',
      success_delete: 'Transaction deleted successfully',
      success_paid: 'Transaction marked as paid',
      invalid_amount: 'Invalid amount',
      invalid_amount_desc: 'Amount must be greater than zero.',
      invalid_desc: 'Invalid description',
      invalid_desc_hint: 'Description is required.',
      reconciliation: 'Reconciliation',
      upload_statement: 'Upload Statement',
      upload_desc: 'Upload bank statements for reconciliation.',
      statements: 'Statements',
      no_statements: 'No statements uploaded.',
      reconciled: 'Reconciled',
      pending_reconciliation: 'Pending Reconciliation',
      overview_pnl: 'Overview & P&L',
      tower_breakdown: 'Tower Breakdown',
      channel_analytics: 'Channel Analytics',
      projected_cash_flow: 'Projected Cash Flow',
      total_expenses: 'Total Expenses',
      net_income: 'Net Income',
      categories: {
        other: 'Other',
        rent: 'Rent',
        maintenance: 'Maintenance',
        cleaning: 'Cleaning',
        hoa: 'HOA / Condo',
        tax: 'Taxes',
        utilities: 'Utilities',
      },
    },
    tasks: {
      title: 'Tasks',
      subtitle: 'Maintenance and service management.',
      board: 'Board',
      list: 'List',
      in_progress: 'In Progress',
      approval: 'Approval',
      status_wait_owner: 'Waiting Owner',
      status_wait_pm: 'Waiting PM',
      task_title: 'Task Title',
      assignee: 'Assignee',
      scheduled_date: 'Scheduled Date',
      service_type: 'Service Type',
      change_status: 'Change Status',
      location: 'Location',
    },
    visits: {
      schedule_visit: 'Schedule Visit',
      enter_details: 'Enter visit details below.',
      assign_team: 'Assign Team Member / Partner',
      time: 'Time',
      reason: 'Reason for Visit',
      showing: 'Showing',
      inspection: 'Inspection',
      client_prefs: 'Client preferences, access codes, etc.',
      list_title: 'Visits List',
      upcoming_past: 'Upcoming and past visits.',
      no_visits: 'No visits scheduled.',
    },
    calendar: {
      title: 'Calendar',
      subtitle: 'Integrated view of events and tasks.',
      integrated_view: 'Integrated View',
      no_activities: 'No activities scheduled for today.',
    },
    messages: {
      title: 'Messages',
      subtitle: 'Unified communication center.',
      new_message: 'New Message',
      select_user: 'Select a User',
      group_team: 'Team',
      group_tenants: 'Tenants',
      group_owners: 'Owners',
      group_partners: 'Partners',
      type_message: 'Type your message...',
      online: 'Online',
      no_contacts: 'No contacts.',
      search_contact: 'Search contact...',
      no_conv: 'No conversations found.',
      new_chat_prompt: 'Start a new conversation',
      select_prompt: 'Select a contact to start chatting.',
      start_chat_desc: 'Start Chat',
    },
    users: {
      title: 'Users',
      subtitle: 'System user and permission management.',
      role_label: 'Role',
      registered: 'Registered',
      registered_desc: 'users registered in the system.',
      permissions: 'Permissions',
      status_active: 'Active',
      status_pending: 'Pending',
      status_blocked: 'Blocked',
      approve_success: 'User approved successfully.',
      block_title: 'Block User',
      block_confirm: 'Are you sure you want to block this user?',
      block_success: 'User blocked successfully.',
    },
    workflows: {
      title: 'Workflows',
      subtitle: 'Process and task automation.',
      new_workflow: 'New Workflow',
      trigger: 'Trigger',
      steps: 'Steps',
      run_manual: 'Run Manually',
      config_workflow: 'Config Workflow',
      edit_workflow: 'Edit Workflow',
      active_status: 'Active Status',
      step_name: 'Step Name',
      step_role: 'Responsible Role',
      add_step: 'Add Step',
      run_workflow: 'Run Workflow',
      select_properties: 'Select Properties',
      tasks_generated_desc:
        'Tasks will be generated for each selected property.',
    },
    renewals: {
      title: 'Contract Renewals',
      subtitle: 'Manage multi-party negotiations and finalize leases.',
      search_placeholder: 'Search renewal...',
      all_status: 'All Status',
      critical_status: 'Critical (≤ 30 days)',
      warning_status: 'Warning (31 - 60 days)',
      safe_status: 'Safe (> 60 days)',
      days_left: '{days} days left',
      current_proposed: 'Current / Proposed',
      negotiation_status: 'Negotiation Status',
      renewed: 'Renewed',
      owner: 'Owner',
      tenant: 'Tenant',
      manage: 'Manage',
      no_results: 'No results found.',
      negotiation_hub: 'Negotiation Hub',
      negotiation_hub_desc: 'Manage multi-party lease renewals efficiently.',
      confidential_note: 'Confidential: Visible only to Property Managers.',
      add_note: 'Add Internal Note',
      mark_vacating: 'Mark as Vacating',
      close_negotiation: 'Close Negotiation',
      close_negotiation_desc:
        'Confirm the new terms and attach the signed contract to finalize. The status will be updated to "Renewed".',
      new_value: 'New Rent Value',
      new_start_date: 'New Start Date',
      new_end_date: 'New End Date',
      contract_upload: 'Upload Contract',
      fill_all_fields: 'Please fill in all fields and attach the contract.',
      both_must_accept:
        'Both Owner and Tenant must accept the terms before closing.',
      contract_renewed: 'Contract renewed successfully.',
      pricing_owner_approval: 'Pricing & Owner Approval',
      current_value: 'Current Value',
      proposed_value: 'Proposed Value',
      update_terms_notify: 'Update Terms & Notify Owner',
      pricing_tenant_approval: 'Pricing & Tenant Approval',
      new_rent_value: 'New Rent Value',
      pending_owner_approval: 'Pending Owner Approval',
      save_decision: 'Save Decision',
      type_message: 'Type a message...',
      empty_notes: 'No notes yet.',
      empty_messages: 'No messages yet.',
    },
    short_term: {
      title: 'Short Term Rental',
      subtitle: 'Manage short-term bookings.',
      new_booking: 'New Booking',
      guest: 'Guest',
      check_in: 'Check-in',
      check_out: 'Check-out',
      total: 'Total',
      platform: 'Platform',
      reports: 'Reports',
      calendar: 'Calendar',
      bookings: 'Bookings',
      communication: 'Communication',
      revenue: 'Revenue',
      occupancy: 'Occupancy',
      auto_replies: 'Auto-Replies',
      send_welcome: 'Send Welcome Message',
      send_checkout_instr: 'Send Check-out Instructions',
      template_name: 'Template Name',
      delay_minutes: 'Delay (minutes)',
      amount: 'Amount',
      direct_booking: 'Direct / Manual',
      full_name_placeholder: 'Full Name',
      email_placeholder: 'guest@email.com',
    },
    common: {
      add_document: 'Add Document',
      document_name: 'Document Name',
      file_url: 'File URL (Mock Upload)',
      no_documents: 'No documents uploaded yet.',
      personal_data: 'Personal Data',
      contact_address: 'Contact & Address',
      documents: 'Documents',
      rg_id: 'ID Document',
      dob: 'Date of Birth / Foundation',
      nationality: 'Nationality',
      marital_status: 'Marital Status',
      profession: 'Profession',
      monthly_income: 'Monthly Income',
      base_rent: 'Base Rent',
      secondary_phone: 'Secondary Phone',
      whatsapp: 'WhatsApp',
      street: 'Street',
      number: 'Number',
      complement: 'Number or complement',
      neighborhood: 'Neighborhood',
      city: 'City',
      state: 'State',
      zip_code: 'Zip Code',
      single: 'Single',
      married: 'Married',
      divorced: 'Divorced',
      widowed: 'Widowed',
      country_us: 'United States',
      country_br: 'Brazil',
      country_es: 'Spain',
      tax_id_label: 'Tax ID / SSN / CPF',
      full_name_label: 'Full Name / Company Name',
      add: 'Add',
      dashboard: 'Real Estate Dashboard',
      real_estate_dashboard: 'Real Estate Dashboard',
      properties: 'Properties',
      tenants: 'Tenants',
      owners: 'Owners',
      partners: 'Partners',
      calendar: 'Calendar',
      tasks: 'Tasks',
      financial: 'Financial',
      financial_hoa: 'Financial HOA',
      invoices: 'Invoices',
      messages: 'Messages',
      settings: 'Settings',
      service_pricing: 'Price Catalog',
      search: 'Search...',
      filter: 'Filter',
      status: 'Status',
      actions: 'Actions',
      save: 'Save',
      cancel: 'Cancel',
      edit: 'Edit',
      delete: 'Delete',
      new: 'New',
      back: 'Back',
      view: 'View',
      details: 'Details',
      name: 'Name',
      email: 'Email',
      phone: 'Phone',
      address: 'Address',
      type: 'Type',
      role: 'Role',
      role_label: 'Role',
      logout: 'Logout',
      profile: 'Profile',
      notifications: 'Notifications',
      team: 'Team',
      automation: 'Automation',
      all: 'All',
      pending: 'Pending',
      accepted: 'Accepted',
      rejected: 'Rejected',
      counter: 'Counter-offer',
      approved: 'Approved',
      completed: 'Completed',
      paid: 'Paid',
      overdue: 'Overdue',
      active: 'Active',
      inactive: 'Inactive',
      occupied: 'Occupied',
      vacant: 'Vacant',
      maintenance: 'Maintenance',
      cleaning: 'Cleaning',
      confirm: 'Confirm',
      upload: 'Upload',
      description: 'Description',
      date: 'Date',
      end_date: 'End Date',
      due_date: 'Due Date',
      value: 'Value',
      priority: 'Priority',
      yes: 'Yes',
      no: 'No',
      none: 'None',
      other: 'Other',
      error: 'Error',
      condominiums: 'Condominiums',
      continue: 'Continue',
      password: 'Password',
      confirm_password: 'Confirm Password',
      add_title: 'Add',
      contact_via_whatsapp: 'WhatsApp',
      contact_via_email: 'Email',
      tax_id: 'Tax ID',
      system_activity: 'System Activity',
      approve: 'Approve',
      reject: 'Reject',
      block: 'Block',
      pending_approval: 'Pending Approval',
      pending_activation: 'Pending Activation',
      blocked: 'Blocked',
      country: 'Country',
      select_country: 'Select Country',
      export_data: 'Export Data',
      export_success: 'Data exported successfully.',
      export_success_title: 'Export Started',
      renewals: 'Contract Renewals',
      short_term: 'Short Term Rental',
      market_analysis: 'Market Analysis',
      advanced_analytics: 'Advanced Analytics',
      automation_rules: 'Automation Rules',
      workflows: 'Workflows',
      portal: 'Portal',
      publicity: 'Publicity',
      login: 'Login',
      register: 'Register',
      welcome: 'Welcome',
      full_name: 'Full Name',
      property: 'Property',
      reports: 'Reports',
      delete_title: 'Are you sure?',
      delete_desc: 'This action cannot be undone.',
      start_now: 'Start Now',
      download: 'Download',
      print: 'Print',
      close: 'Close',
      delete_active_tenant_error:
        'Cannot delete property with an active tenant.',
      confirm_delete: 'Confirm Delete',
      operations: 'Operations',
      contracts: 'Contracts',
      total: 'Total',
      done: 'Done',
      taxes: 'Taxes',
      utilities: 'Utilities',
      inventory: 'Inventory',
      history: 'History',
      activity_log: 'Activity Log',
      created_at: 'Created at',
      updated_at: 'Updated at',
      select: 'Select',
      required: 'Required',
      success: 'Success',
      removed: 'Removed',
      copied: 'Copied',
      empty: 'Empty',
      relationships: 'Relationships',
      relationship_owner: 'Owner',
      relationship_tenant: 'Tenant',
      no_owner: 'No Owner',
      add_owner: 'Add Owner',
      no_active_tenant: 'No Active Tenant',
      export: 'Export',
      visits: 'Visits',
      schedule_visit: 'Schedule Visit',
      schedule_maintenance: 'Schedule Maintenance',
      client_name: 'Client Name',
      loading: 'Loading...',
      unknown: 'Unknown',
      resource: 'Resource',
      profile: 'Profile',
      visit_date: 'Visit Date',
      visit_status: 'Visit Status',
      scheduled: 'Scheduled',
      canceled: 'Canceled',
      visit_scheduling: 'Visit Scheduling',
      visits_list: 'Visits List',
      total_revenue: 'Total Revenue',
      sold: 'Sold',
      sale_pending: 'Sale Pending',
      category: 'Category',
      quantity: 'Quantity',
      condition: 'Condition',
      revenue: 'Revenue',
      profit: 'Profit',
      provider: 'Provider',
      analytics: {
        benchmark_title: 'Market Benchmark',
        benchmark_desc: 'Performance comparison with market data.',
        house_model: 'House Model',
        internal_perf: 'Internal Perf',
        market_avg: 'Market Avg',
        occupancy: 'Occupancy',
        profitability_title: 'Profitability',
        profit: 'Profit',
        revenue: 'Revenue',
        adr: 'Avg Daily Rate (ADR)',
        privacy_mode: 'Privacy Mode',
      },
      help_hub: 'Help Hub',
      hotels: 'Hotels',
      location: 'Location',
      access_credentials: 'Access Credentials',
      manager: 'Manager',
      contact: 'Contact',
      history_adjustments: 'Adjustment History',
      name_required: 'Name is required',
      email_invalid: 'Invalid email',
      error_404_title: '404',
      error_404_desc: 'Oops! Page not found.',
      return_home: 'Return Home',
      access_denied: 'Access Denied',
      access_denied_desc: 'You do not have permission to view this page.',
      list: 'List',
      calendar_view: 'Calendar',
      start_date: 'Start Date',
      and: 'and',
      validation_error: 'Validation Error',
      delete_success: 'Deleted successfully.',
      error_delete: 'Error deleting.',
      delete_linked_error: 'Cannot delete because there are linked records.',
      new_task: 'New Task',
      create_title: 'Create Task',
      search_address: 'Search Address',
      manager_col: 'Manager',
    },
    dashboard: {
      title: 'Central Control Panel',
      desc_main:
        'Complete managerial view: Performance, Financials, Hospitality and Operations.',
      integrated_management: 'Integrated Management',
      tabs: {
        overview: 'Overview',
        financial: 'Financial',
        hospitality: 'Hospitality',
        operations: 'Operations & Team',
      },
      cards: {
        total_revenue: 'Total Revenue (Month)',
        occupancy_rate: 'Occupancy Rate',
        default_rate: 'Default Rate',
        portfolio: 'Portfolio',
        vs_last_month: 'vs last month',
        occupied_properties: 'occupied properties',
        pending_amount: 'pending',
        active_properties: 'Properties under active management',
      },
      charts: {
        cash_flow: 'Consolidated Cash Flow',
        cash_flow_desc: 'Revenues and Expenses over time',
        portfolio_status: 'Portfolio Status',
        portfolio_status_desc: 'Occupancy distribution',
        revenue: 'Revenue',
        expense: 'Expense',
        net_profit: 'Net Profit',
      },
      financial_tab: {
        noi: 'Net Operating Income (NOI)',
        this_month: 'this month',
        total_expenses: 'Total Expenses',
        above_target: '(Above target)',
        profit_margin: 'Profit Margin',
        industry_avg: 'Healthy industry average: 50-60%',
        revenue_growth: 'Revenue Growth (Continuous Evolution)',
        gross_revenue: 'Gross Revenue',
      },
      hospitality_tab: {
        active_hotels: 'Active Hotels',
        bookings_today: 'Bookings (Today)',
        adr: 'ADR (Average Daily Rate)',
        revpar: 'RevPAR',
        metrics_6m: 'Hospitality Metrics (Last 6 months)',
        occupancy_pct: 'Occupancy (%)',
      },
      operations_tab: {
        pending_tasks: 'Pending Tasks',
        completed_tasks: 'Completed Tasks',
        active_tenants: 'Active Tenants',
        pending_renewals: 'Pending Renewals',
        tenant_profile: 'Tenant Profile',
        latest_issues: 'Latest Issues',
        maintenance_ac: 'AC Maintenance',
        unit: 'Unit',
      },
      demographics: {
        long_term: 'Long Term',
        short_term: 'Short Term',
        commercial: 'Commercial',
      },
      status: {
        rented: 'Rented',
        available: 'Available',
        maintenance: 'Maintenance',
      },
      months: [
        'Jan',
        'Feb',
        'Mar',
        'Apr',
        'May',
        'Jun',
        'Jul',
        'Aug',
        'Sep',
        'Oct',
        'Nov',
        'Dec',
      ],
      unread: 'unread',
      from_last_month: 'from last month',
      customize: 'Customize',
      customize_view: 'Customize View',
      customize_desc: 'Select widgets to display.',
      kpi_indicators: 'KPI Indicators',
      revenue_chart: 'Revenue Chart',
      revenue_vs_expenses: 'Revenue vs Expenses',
      revenue_overview: 'Financial overview for recent months.',
      quick_calendar: 'Quick Calendar',
      todays_activity: "Today's activities.",
      pending_approvals: 'Pending Approvals',
      pending_desc: 'Items requiring your attention.',
      review: 'Review',
      approve: 'Approve',
      expense_distribution: 'Expense Distribution',
      expense_desc: 'Breakdown of expenses by category.',
      subtitle: 'Welcome back! Here is your operations summary.',
    },
    gamification: {
      health_score: 'Health Score',
      portfolio_health: 'Portfolio Health',
      desc: 'Score based on occupancy, maintenance, and reviews.',
      level: 'Level',
      expert: 'Expert',
      target: 'Target',
    },
    properties: {
      title: 'Properties',
      subtitle: 'Manage your real estate portfolio.',
      new_property: 'New Property',
      add_title: 'Add Property',
      add_description: 'Fill out the form to create a new property.',
      rental_type: 'Rental Type',
      profile_short: 'Short Term',
      profile_long: 'Long Term',
      search_placeholder: 'Search property...',
      address_placeholder: 'Property address',
      zip_code: 'Zip Code',
      zip_required: 'Zip Code is required',
      info_label: 'Additional Info',
      city_placeholder: 'City',
      state_placeholder: 'State',
      property_value: 'Property Value',
      hoa_fee: 'HOA Fee',
      hoa_auto_hint: 'Monthly auto-calculation',
      cover_image: 'Cover Image',
      no_image_selected: 'No image selected',
      no_image: 'No Image',
      view_details: 'View Details',
      property_added: 'Property Added',
      validation_error: 'Validation Error',
      name_required: 'Name/Nickname is required',
      address_required: 'Address is required',
      condominium_link: 'Condominium Link',
      select_condominium: 'Select Condominium',
      condo_inherited_info: 'Access and contact details will be inherited.',
      property_nickname: 'Property Nickname / Name',
      nickname_placeholder: 'E.g. Beach House, Villa Sol',
      photos: 'Photos (Select Cover)',
      cover: 'Cover',
      city_state_required: 'City and State are required',
      profile_required: 'Profile Type is required',
      delete_success: 'Property deleted successfully.',
      error_delete: 'Error deleting property.',
      all_profiles: 'All Profiles',
      profile_filter: 'Profile Filter',
      monthly: 'Monthly',
      quarterly: 'Quarterly',
      annually: 'Annually',
      hoa_freq: 'HOA Frequency',
      overview: 'Overview',
      house: 'House',
      condo: 'Condo',
      townhouse: 'Townhouse',
      marketing: 'Marketing',
      publish_portals: 'Publish to Portals',
      upload_image: 'Upload Image',
      independent_community: 'Independent',
      additional_info_placeholder: 'Apt, Block, etc.',
      tabs: {
        overview: 'Overview',
        analytics: 'Analytics',
        reports: 'Reports',
        maintenance: 'Maintenance',
        contracts: 'Contracts',
        inventory: 'Inventory',
        features: 'Features',
        location: 'Location',
        sync: 'Sync',
        financial: 'Financial',
        marketing: 'Marketing',
        content: 'Content',
        documents: 'Documents',
        logs: 'Logs',
        pricing: 'Pricing',
      },
      location: {
        address: 'Address',
        neighborhood: 'Neighborhood',
        community: 'Community',
        linked_condo: 'Linked Condominium',
        complement_placeholder: 'Apt, Block, Reference',
        country_us: 'United States (USA)',
        country_br: 'Brazil',
        country_es: 'Spain',
      },
      contacts: {
        new_contact: 'New Contact',
        role_concierge: 'Concierge',
        role_maintenance: 'Maintenance',
        role_emergency: 'Emergency',
        role_local_agent: 'Local Agent',
      },
      content: {
        house_rules: 'House Rules',
        auto_translate: 'Auto-Translate',
        copy_from: 'Copy from',
        desc_pt_placeholder: 'Full description in Portuguese...',
        desc_en_placeholder: 'Full description in English...',
        desc_es_placeholder: 'Full description in Spanish...',
        rules_pt_placeholder: 'HOA Rules in Portuguese...',
        rules_en_placeholder: 'HOA Rules...',
        rules_es_placeholder: 'HOA Rules in Spanish...',
      },
      financial: 'Financial',
      financial_fields: {
        name_placeholder: 'e.g. Internet',
        provider_placeholder: 'e.g. Duke Energy',
        account_placeholder: 'Account ID',
      },
      features: {
        bedrooms: 'Bedrooms',
        bathrooms: 'Bathrooms',
        guests: 'Guests',
        wifi_connectivity: 'Wi-Fi Connectivity',
        ssid: 'SSID / Network Name',
        password: 'Password',
        access_codes: 'Access Codes',
        unit_code: 'Unit Code',
        building_code: 'Building Code',
        pool_code: 'Pool Code',
        staff_code: 'Staff Code',
        condo_read_only: 'Condominium Access (Read Only)',
        condo_hint: 'Credentials managed in {condo} settings.',
      },
    },
    auth: {
      login_title: 'Login',
      login_desc: 'Sign in to your account to continue',
      email_placeholder: 'example@company.com',
      forgot_password: 'Forgot password?',
      no_account: 'No account?',
      demo_login: 'Demo Login',
      register_title: 'Create Account',
      register_desc: 'Start managing your properties today',
      name_placeholder: 'Your full name',
      company_details: 'Company Details',
      company_placeholder: 'Your company name',
      tax_id_placeholder: 'Tax ID / VAT',
      address_placeholder: 'Business address',
      already_have_account: 'Already have an account?',
    },
    settings: {
      title: 'Settings',
      subtitle: 'Manage your account and system settings.',
      personal_info: 'Personal Info',
      update_info: 'Update your information.',
      change_photo: 'Change Photo',
      full_name: 'Full Name',
      save_changes: 'Save Changes',
      integrations: 'Integrations',
      billing_payment: 'Billing and Payment',
      billing_gateways_title: 'Payment Gateways',
      billing_gateways_desc: 'Configure accepted payment methods.',
      payment_gateways: 'Payment Gateways',
      system_alerts: 'System Alerts',
      system_alerts_desc: 'Configure automated notifications.',
      automation_title: 'Automation Rules',
      automation_desc:
        'Manage and configure automation rules for tasks, finance, and communication.',
      rent_reminder: 'Rent Reminder',
      days_before: 'Days Before',
      company_legal_name: 'Legal Name',
    },
    status: {
      rented: 'Rented',
      available: 'Available',
      sold: 'Sold',
      occupied: 'Occupied',
      vacant: 'Vacant',
      suspended: 'Suspended',
      maintenance: 'Maintenance',
      reserved: 'Reserved',
      rescheduled: 'Rescheduled',
      released: 'Released',
      in_registration: 'In Registration',
      interested: 'Interested',
      negotiating: 'Negotiating',
      owner_contacted: 'Owner Contacted',
      tenant_contacted: 'Tenant Contacted',
      vacating: 'Vacating',
    },
    roles: {
      platform_owner: 'Admin',
      software_tenant: 'Manager',
      internal_user: 'Staff',
      partner: 'Partner',
      property_owner: 'Owner',
      tenant: 'Tenant',
      partner_employee: 'Partner Employee',
    },
    marketing_tab: {
      portal_sync: 'Portal Sync',
      manage_listings: 'Manage listings on external portals.',
      publish_btn: 'Publish',
      publishing: 'Publishing...',
      publish_success: 'Publishing started successfully.',
      leads: 'Leads & Prospects',
      inquiries_desc: 'Manage inquiries from potential guests.',
      contact_info: 'Contact Info',
      source: 'Source',
      inquiry_date: 'Inquiry Date',
      status: 'Status',
      zillow_desc: 'US Market Leader',
      idealista_desc: 'European Market',
    },
    owner_portal: {
      welcome: 'Welcome',
      subtitle: 'Owner Asset Portal',
      registered_properties: 'Registered Properties',
      pending_tasks: 'Pending Task Approvals',
      contact_pm: 'Contact PM',
      pending_hoa: 'Pending HOA & Taxes',
      pending_approvals: 'Pending Cost Approvals',
      pending_renewals: 'Pending Renewals',
      ledger: 'Ledger & Financial',
      hoa_taxes: 'HOA & Taxes',
      damages_costs: 'Damages & Costs',
      renewals: 'Renewals',
      properties: 'Properties',
      hoa_desc: 'Manage your upcoming HOA fees and property taxes.',
      go_to_financial: 'Go to Financial',
      no_records: 'No records found',
      no_hoa_records:
        'No HOA or Tax records have been registered for your properties.',
      sandbox_tasks: 'Sandbox Tasks (Development Only)',
      long_term_renewals: 'Long-term Contract Renewals',
      no_pending_renewals: 'No pending renewals',
      no_pending_renewals_desc:
        'There are no long-term contracts awaiting your approval at this time.',
      current_rent: 'Current Rent',
      proposed_rent: 'Proposed Rent',
      cost_approvals: 'Cost Approvals (Above Limit)',
      estimated_cost: 'Estimated Cost',
      suggested_date: 'Suggested Date',
      maintenance_history: 'Damages and Maintenance Costs History',
      no_tasks:
        "No tasks recorded for this owner's properties in the selected period.",
      task_approved: 'Task Approved',
      task_rejected: 'Task Rejected',
      task_status_updated: 'Task status updated successfully.',
    },
    automation: {
      title: 'Automation',
      quickbooks_export: 'Export to QuickBooks',
      quickbooks_desc: 'Download QB compatible transactions.',
      export_csv: 'Export CSV',
      export_success_title: 'Export Started',
      auto_generate_invoice: 'Auto Generate Invoice',
      generate_invoice: 'Generate Invoice',
      auto_approval: 'Auto-Approval',
      auto_approval_desc:
        'Automatically approve maintenance tasks below a certain cost.',
      enable_auto_approval: 'Enable Auto-Approval',
      billing_automation: 'Billing Automation',
      billing_automation_desc:
        'Generate invoices automatically based on events.',
      auto_generate_invoices: 'Auto-Generate Invoices',
      rent_reminders_desc: 'Send automated reminders to tenants.',
      enable_reminders: 'Enable Reminders',
      rule_enabled: 'Rule enabled',
      max_cost_threshold: 'Max Cost Threshold',
      tasks_below_threshold:
        'Tasks below this amount will be approved instantly.',
      trigger_event: 'Trigger Event',
      confirm_export: 'Confirm Export',
      triggering: 'Triggering',
      action: 'Action',
      status: 'Automation Status',
      trigger_desc:
        'Invoices will be drafted when a task status changes to "Completed".',
      auto_sms_desc: 'Automatic email/SMS will be sent.',
      select_export_scope: 'Select Export Scope',
      choose_data_range: 'Choose the data range for the QuickBooks export.',
      data_scope: 'Data Scope',
      general_activities: 'General Activities',
      by_owner: 'By Owner',
      by_partner: 'By Partner',
      task_automation: 'Task Automation',
      financial_automation: 'Financial Automation',
      communication_automation: 'Communication Automation',
      quickbooks_integration: 'QuickBooks Integration',
    },
  },
  es: {
    sync: {
      title: 'Sincronización Básica iCal',
      sync_desc: 'Sincronización simple de calendario vía URL.',
      platform: 'Plataforma',
      ical_url: 'URL iCal',
      add_link: 'Añadir Enlace',
      status: 'Estado',
      last_sync: 'Última Sincronización',
      no_links: 'Ningún enlace configurado.',
      sync_now: 'Sincronizar Ahora',
      sync_success: 'Sincronización Iniciada',
      export_calendar: 'Exportar Calendario',
      copy_link: 'Copiar Enlace',
      airbnb: 'Airbnb (iCal)',
      booking: 'Booking.com (iCal)',
      vrbo: 'Vrbo (iCal)',
    },
    publicity: {
      title: 'Administración de Publicidad',
      subtitle:
        'Gestione anunciantes, campañas y configuraciones de monetización.',
      tab_ads: 'Anuncios',
      tab_advertisers: 'Anunciantes',
      tab_pricing: 'Config. de Precios',
      advertiser_list: {
        title: 'Anunciantes',
        search_placeholder: 'Buscar anunciantes...',
        add_btn: 'Añadir Anunciante',
        table_company: 'Empresa / Nombre',
        table_contact: 'Contacto',
        table_address: 'Dirección',
        table_registered: 'Registrado',
        empty_state: 'Ningún anunciante encontrado.',
        modal_edit: 'Editar Anunciante',
        modal_new: 'Nuevo Anunciante',
        label_company: 'Empresa / Nombre *',
        label_email: 'Email *',
        label_phone: 'Teléfono',
        label_address: 'Dirección',
        delete_confirm: '¿Estás seguro de que deseas eliminar este anunciante?',
        delete_success: 'Anunciante eliminado.',
        update_success: 'Anunciante actualizado con éxito.',
        add_success: 'Anunciante añadido con éxito.',
        validation_error: 'Nombre y email son obligatorios.',
      },
      ads_manager: {
        title: 'Gestor de Anuncios',
        search_placeholder: 'Buscar anuncios...',
        add_btn: 'Nuevo Anuncio',
        table_ad_info: 'Info del Anuncio',
        table_advertiser: 'Anunciante',
        table_placement: 'Posicionamiento',
        table_validity: 'Validez',
        table_price: 'Precio',
        table_status: 'Estado',
        empty_state: 'Ningún anuncio encontrado.',
        modal_edit: 'Editar Anuncio',
        modal_new: 'Nuevo Anuncio',
        label_title: 'Título *',
        label_advertiser: 'Anunciante *',
        select_advertiser: 'Seleccione anunciante',
        label_placement: 'Lugar de Posicionamiento',
        label_price: 'Precio',
        label_start_date: 'Fecha de Inicio',
        label_end_date: 'Fecha de Fin',
        label_image_url: 'URL de la Imagen *',
        label_target_link: 'URL del Enlace de Destino',
        active_status: 'Estado Activo',
        active_status_desc:
          'Alternar la visibilidad del anuncio en la plataforma.',
        save_ad: 'Guardar Anuncio',
        delete_confirm: '¿Estás seguro de que deseas eliminar este anuncio?',
        delete_success: 'Anuncio eliminado.',
        update_success: 'Anuncio actualizado con éxito.',
        add_success: 'Anuncio creado con éxito.',
        validation_error:
          'Título, URL de Imagen y Anunciante son obligatorios.',
        link: 'Enlace',
        active: 'Activo',
        inactive: 'Inactivo',
        unknown: 'Desconocido',
        placements: {
          home_top: 'Banner Superior Inicio',
          home_bottom: 'Banner Inferior Inicio',
          partner_page: 'Página del Socio',
          tenant_page: 'Portal del Inquilino',
          pm_login: 'Pantalla de Login del PM',
          sidebar: 'Barra Lateral',
          footer: 'Pie de Página',
        },
      },
      pricing_config: {
        title_base: 'Tarifas Base por Duración',
        desc_base:
          'Establezca los precios estándar para diferentes duraciones de anuncios.',
        label_weekly: 'Tarifa Semanal',
        label_biweekly: 'Tarifa Quincenal (14 Días)',
        label_monthly: 'Tarifa Mensual',
        title_modifiers: 'Modificadores de Posicionamiento',
        desc_modifiers: 'Costos adicionales para posicionamientos premium.',
        tooltip_modifiers:
          'Estos valores representam adiciones fijas a la tarifa base de duración dependiendo de dónde se coloque el anuncio.',
        save_btn: 'Guardar Configuración',
        save_success_title: 'Configuración de Precios Guardada',
        save_success_desc:
          'Tus tarifas de anuncios han sido actualizadas con éxito.',
      },
    },
    sidebar: {
      dashboard: 'Panel',
      portfolio: 'Portafolio',
      units: 'Propiedades',
      condominiums: 'Condominios',
      owners: 'Propietarios',
      tenants: 'Inquilinos',
      financial: 'Financiero',
      marketing: 'Marketing',
      reports: 'Informes',
      settings: 'Configuraciones',
      performance: 'Rendimiento',
      guest_services: 'Servicios Huésped',
      pos: 'Punto de Venta',
      main_menu: 'Menú Principal',
      system: 'Sistema',
      migration_hub: 'Migración',
      publicity_admin: 'Publicidad',
      users: 'Usuarios',
      tenant_portal: 'Portal Inquilino',
      owner_portal: 'Portal Propietario',
      partner_portal: 'Portal Socio',
      housekeeping: 'Limpieza',
      night_audit: 'Auditoría Nocturna',
      calendar: 'Calendario',
      partners: 'Socios',
    },
    housekeeping: {
      title: 'Limpieza',
      subtitle: 'Gestione tareas de limpieza y programaciones.',
      task_title: 'Título de la Tarea',
      property: 'Propiedad',
      assignee: 'Responsable',
      date: 'Fecha',
      status: 'Estado',
    },
    hotel_form: {
      hotel_name: 'Nombre del Hotel',
      street_address: 'Dirección',
      number: 'Número',
      neighborhood: 'Barrio',
      city: 'Ciudad',
      state: 'Estado / Provincia',
      zip_code: 'Código Postal',
      towers_wings: 'Torres / Alas',
      add_division: 'Añadir División',
      define_divisions: 'Defina divisiones internas como Torres o Alas.',
      validation_error: 'Nombre y Ciudad son obligatorios.',
      placeholder_name: 'Ej: Grand Heritage Hotel',
      details: 'Detalles',
      location: 'Ubicación',
      no_divisions:
        'Ninguna división añadida. Haga clic en "Añadir División" para crear una.',
      delete_title: 'Eliminar Hotel',
      delete_desc:
        'Esta acción no se puede deshacer. ¿Está seguro de que desea eliminar este hotel permanentemente?',
    },
    hotels: {
      title: 'Hoteles',
      subtitle: 'Gestión de hoteles y habitaciones.',
      add_title: 'Añadir Hotel',
      new_hotel: 'Nuevo Hotel',
      towers: 'Torres',
      rooms: 'Habitaciones',
      floors: 'Pisos',
      add_tower: 'Añadir Torre',
      new_tower: 'Nueva Torre',
      hotel_details: 'Detalles del Hotel',
      no_hotels: 'Ningún hotel encontrado.',
      not_found: 'Hotel No Encontrado',
      update_success: 'Hotel actualizado con éxito.',
      delete_success: 'Hotel eliminado.',
      tower_add_success: 'Torre añadida con éxito.',
      all_rooms: 'Todas las Habitaciones',
      info: 'Información del Hotel',
      towers_wings: 'Torres y Alas',
      add_new_tower: 'Añadir Nueva Torre/Ala',
      tower_name: 'Nombre de la Torre',
      manage_rooms: 'Gestionar Habitaciones',
      no_towers: 'Ninguna torre o ala definida.',
    },
    header: {
      demo_profiles: 'Perfiles Demo',
      other_users: 'Otros Usuarios',
    },
    market_analysis: {
      title: 'Análisis de Mercado',
      subtitle: 'Benchmark de mercado y tendencias.',
      demand_index: 'Índice de Demanda',
      peak_season: 'Temporada alta aproximándose',
      comp_set_rank: 'Clasificación del Set Competitivo',
      rank_desc: 'De 12 hoteles locales',
      competitor_pricing: 'Precios de la Competencia',
      demand_forecast: 'Previsión de Demanda',
      no_forecast: 'Ninguna previsión disponible.',
      avg_occupancy: 'Tasa de Ocupación Media',
      avg_daily_rate: 'Tarifa Media Diaria (ADR)',
      spending_trends: 'Tendencias de Gastos',
      high: 'Alta',
      medium: 'Media',
      low: 'Baja',
      historical_view: 'Visión histórica del rendimiento del mercado',
      avg_rate_comparison: 'Comparación de tarifa media diaria',
      projected_demand: 'Niveles de demanda proyectados para fechas futuras',
    },
    pos: {
      title: 'Punto de Venta',
      subtitle: 'Gestión de productos, ventas y stock.',
      new_product: 'Nuevo Producto',
      manage_products: 'Gestionar Productos',
      product_name: 'Nombre del Producto',
      price: 'Precio',
      category: 'Categoría',
      validity_start: 'Validez Inicial',
      active: 'Activo',
      inactive: 'Inactivo',
      stock: 'Stock',
      actions: 'Acciones',
      create: 'Crear',
      edit: 'Editar',
      inactivate: 'Inactivar',
      delete: 'Eliminar',
      confirm_delete: 'Confirmar Eliminación',
      delete_desc: '¿Estás seguro de que deseas eliminar este producto?',
      save_success: 'Producto guardado con éxito.',
      add_to_cart: 'Añadir al Carrito',
      checkout: 'Finalizar Venta',
      charge_room: 'Cargar a la Habitación',
      out_of_stock: 'Sin Stock',
      cart_empty: 'Carrito Vacío',
      transaction_success: '¡Venta realizada con éxito!',
      laundry_service: 'Servicio de Lavandería',
      laundry: 'Lavandería',
      club_sandwich: 'Club Sándwich',
      restaurant: 'Restaurante',
      minibar: 'Minibar',
      cola: 'Refresco',
      chips: 'Patatas Fritas',
      guest: 'Huésped',
      select: 'Seleccione',
    },
    guest_services: {
      title: 'Servicios al Huésped',
      subtitle: 'Catálogo de servicios y programación.',
      new_service: 'Nuevo Servicio',
      manage_services: 'Gestionar Servicios',
      financial_insights: 'Insights Financieros',
      service_name: 'Nombre del Servicio',
      description: 'Descripción',
      price: 'Precio',
      category: 'Categoría',
      validity_start: 'Validez Inicial',
      seasonal_pricing: 'Precios por Temporada',
      add_seasonal_price: 'Añadir Precio de Temporada',
      start_date: 'Fecha Inicio',
      end_date: 'Fecha Fin',
      schedule: 'Programar',
      schedule_for: 'Programar Para',
      assign_service: 'Asignar Servicio',
      service_assigned: 'Servicio asignado con éxito.',
      save_success: 'Servicio guardado con éxito.',
    },
    performance: {
      guest_reviews: 'Reseñas de Huéspedes',
      average_rating: 'Calificación Media',
      based_on_reviews: 'Basado en {count} reseñas',
      response: 'Respuesta',
      no_reviews: 'Ninguna reseña aún.',
      rating_reviews_trend: 'Tendencia de Calificaciones y Comentarios',
      guest_satisfaction_evolution:
        'Evolución de la satisfacción de los huéspedes a lo largo del tiempo.',
      financial_performance: 'Rendimiento Financiero (RevPAR)',
      revpar_trends: 'Tendencias de Ingresos por Habitación Disponible.',
    },
    marketing: {
      promotions: 'Promotions',
      campaigns: 'Campañas',
      automation: 'Automatización',
      leads: 'Leads e Interesados',
      new_code: 'Nuevo Código',
      create_promo: 'Crear Código Promocional',
      discount: 'Descuento',
      validity: 'Validez',
      usage: 'Uso',
      new_campaign: 'Nueva Campaña',
      create_campaign: 'Crear Campaña',
      target_audience: 'Público Objetivo',
      code: 'Código',
      automated_workflows: 'Flujos Automatizados',
      automated_desc: 'Disparar correos electrónicos basados en eventos.',
      email_templates: 'Plantillas de Correo',
      email_desc: 'Contenido reutilizable con variables.',
      trigger_event: 'Evento Desencadenante',
      timing: 'Tiempo',
      template: 'Plantilla',
      subject: 'Asunto',
      body: 'Cuerpo',
      placeholders: 'Variables disponibles',
      active: 'Activo',
      booking_confirmed: 'Reserva Confirmada',
      check_in: 'Check-in',
      check_out: 'Check-out',
      cancellation: 'Cancelación',
      immediately: 'Inmediatamente',
      hours: 'horas',
      before: 'antes',
      after: 'después',
      new_message: 'Nueva Mensaje',
      new_workflow: 'Nuevo Flujo de Trabajo',
    },
    analytics_page: {
      occupancy_comparison: 'Comparación de Ocupación',
      profitability_comparison: 'Comparación de Rentabilidad',
      internal_performance: 'Rendimiento Interno',
      market_average: 'Promedio del Mercado',
      benchmark_desc: 'Comparativo de rendimiento con el mercado.',
    },
    reports: {
      title: 'Informes Avanzados',
      subtitle:
        'Análisis de inventario, mantenimiento y condiciones de las propiedades.',
      period: 'Período',
      property: 'Propiedad',
      apply_filters: 'Aplicar Filtros',
      maintenance_analytics: 'Análisis de Mantenimiento',
      inventory_health: 'Salud del Inventario',
      maintenance_efficiency: 'Eficiencia de Mantenimiento',
      inventory_condition: 'Condición del Inventario',
      damage_by_property: 'Daños por Propiedad',
      total_items: 'Total de Artículos',
      damaged_poor: 'Dañado/Malo',
      total_spend: 'Gasto Total',
      total_tasks: 'Total de Tareas',
      avg_resolution: 'Resolución Media',
      avg_cost_task: 'Custo Medio/Tarea',
      cost_breakdown: 'Desglose de Costos',
      distribution_expenses: 'Distribución de Gastos',
      monthly_expenditure: 'Gasto Mensual',
      spending_trends: 'Tendencias de Gastos',
      reports: 'Informes',
    },
    service_pricing: {
      title: 'Catálogo de Precios',
      desc: 'Catálogo de precios para servicios y productos genéricos.',
      subtitle: 'Gestione costos y precios de venta.',
      catalog_tab: 'Catálogo de Preços',
      analytics_tab: 'Informes',
      new_rate: 'Nueva Tarifa',
      service_name: 'Nombre del Servicio',
      partner_payment: 'Pago Socio',
      pm_value: 'Valor PM',
      product_price: 'Precio de Venda',
      cost_price: 'Precio de Costo',
      valid_from: 'Válido Desde',
      valid_to: 'Válido Hasta',
      margin: 'Margen',
      distribution: 'Distribución',
      profit_analysis: 'Análisis de Beneficios',
    },
    condominiums: {
      title: 'Condominios',
      subtitle: 'Gestión de condominios y asociaciones.',
      new_condo: 'Nuevo Condominio',
      add_title: 'Añadir Condominio',
      manager: 'Gerente/Administrador',
      contact: 'Contacto',
      no_condos: 'Ningún condominio encontrado.',
      general_info: 'Información General',
      addressing: 'Dirección',
      access_codes: 'Códigos de Acceso',
      management: 'Gestión',
      description: 'Descripción',
      street: 'Calle/Avenida',
      number: 'Número',
      complement: 'Número o complemento',
      neighborhood: 'Barrio',
      city: 'Ciudad',
      state: 'Estado',
      zip_code: 'Código Postal',
      manager_name: 'Nombre del Gerente',
      manager_phone: 'Teléfono del Gerente',
      manager_email: 'Email del Gerente',
      gate_code: 'Código del Portón',
      pedestrian_gate: 'Puerta de Peatones',
      pool_code: 'Código de Piscina',
      amenities_code: 'Código de Áreas Comunes',
      edit_condo: 'Editar Condominio',
      view_details: 'Ver Detalles',
      search_placeholder: 'Buscar condominio...',
      delete_confirm: '¿Estás seguro de que deseas eliminar este condominio?',
    },
    owners: {
      title: 'Propietarios',
      subtitle: 'Base de propietarios e inversores.',
      new_owner: 'Nuevo Propietario',
      register_title: 'Registro de Propietario',
      base_title: 'Base de Propietarios',
      search_placeholder: 'Buscar propietario...',
      contact_details: 'Detalles de Contacto',
      properties_count: 'Propiedades',
      workflow_started: 'Flujo de Trabajo Iniciado',
      workflow_desc: 'Acción {action} iniciada para {name}.',
      renew_contract: 'Renovar Contrato',
      success_desc: 'Propietario registrado con éxito.',
      add_owner: 'Añadir Propietario',
      edit_owner: 'Editar Propietario',
      owner_added: 'Propietario añadido con éxito',
      owner_updated: 'Propietario actualizado con éxito',
      delete_owner: 'Eliminar Propietario',
      delete_owner_desc: 'Esta acción no se puede deshacer.',
    },
    tenants: {
      title: 'Inquilinos',
      subtitle: 'Gestión de contratos e inquilinos.',
      new_tenant: 'Nuevo Inquilino',
      register_title: 'Registro de Inquilino',
      list_title: 'Lista de Inquilinos',
      search_placeholder: 'Buscar inquilino...',
      property: 'Propiedad',
      send_message: 'Enviar Mensaje',
      add_tenant: 'Añadir Inquilino',
      edit_tenant: 'Editar Inquilino',
      tenant_added: 'Inquilino añadido con éxito',
      tenant_updated: 'Inquilino actualizado con éxito',
      delete_tenant: 'Eliminar Inquilino',
      delete_tenant_desc: 'Esta acción no se puede deshacer.',
    },
    partners: {
      title: 'Socios',
      subtitle: 'Proveedores y prestadores de servicios.',
      new_partner: 'Nuevo Socio',
      register_title: 'Registro de Socio',
      no_partners: 'Ningún socio encontrado.',
      search_placeholder: 'Buscar socio...',
      contact_name: 'Nombre del Contacto',
      company_name: 'Nombre de la Empresa',
      category: 'Categoría',
      agent: 'Agente',
      cleaning: 'Limpieza',
      maintenance: 'Mantenimiento',
      bank_info: 'Datos Bancarios',
      bank_name: 'Banco',
      routing: 'Agencia/Routing',
      account: 'Cuenta',
      payment_history: 'Historial de Pagos',
      financial_report: 'Informe Financiero',
      inspección: 'Inspección',
    },
    invoices: {
      title: 'Facturas',
      subtitle: 'Gestión de cobros y pagos.',
      create_new: 'Crear Nueva',
      invoice_id: 'ID Factura',
      amount: 'Importe',
      bill_to: 'Facturar A',
      status_paid: 'Pagado',
      status_pending: 'Pendiente',
      status_approved: 'Aprobado',
      no_invoices: 'Ninguna factura encontrada.',
      send_to_payer: 'Enviar al Pagador',
      sent_success: 'Factura enviada con éxito.',
      view: 'Ver',
      invoice_viewer: {
        title: 'Visor de Factura',
        invoice_no: 'Factura Nº',
        date: 'Fecha',
        bill_to: 'Facturar A',
        description: 'Descripción',
        quantity: 'Cant',
        amount: 'Importe',
        subtotal: 'Subtotal',
        total: 'Total',
        thank_you: '¡Gracias por su preferencia!',
        print: 'Imprimir',
        download: 'Descargar',
      },
    },
    financial: {
      title: 'Financiero',
      subtitle: 'Visión general financiera y conciliación.',
      management_desc:
        'Gestión de cuentas corrientes, costos fijos y variables.',
      add_transaction: 'Añadir Transacción',
      add_title: 'Incluir Transacción',
      export_csv: 'Exportar (CSV)',
      type: 'Tipo',
      date: 'Fecha',
      description: 'Descripción',
      category: 'Categoría',
      amount: 'Monto',
      property_optional: 'Propiedad (Opcional)',
      property: 'Propiedad',
      cost_category: 'Categoría de Costo',
      is_recurring: '¿Es un gasto/ingreso recurrente?',
      recurrence_freq: 'Frecuencia de Recurrencia',
      recurring_hint:
        'Al marcar este ítem como "Pagado", el sistema generará automáticamente la entrada para el próximo mes/año.',
      initial_status: 'Estado Inicial',
      save_transaction: 'Guardar Transacción',
      balances_panel: 'Panel de Saldos y Reportes',
      balances_desc:
        'Vea el saldo de la cuenta corriente filtrado por su preferencia.',
      pm_general: 'PM (General)',
      owner: 'Propietario',
      select_owner: 'Seleccione Propietario',
      all_owners: 'Todos los Propietarios',
      all_properties: 'Todas las Propiedades',
      viewing_your_properties: 'Viendo Sus Propiedades',
      filters: 'Filtros:',
      all_categories: 'Todas las Categorías',
      incomes: 'Ingresos',
      expenses: 'Gastos',
      maintenances: 'Mantenimientos',
      cleanings: 'Limpiezas',
      hoa_condo: 'HOA / Condominio',
      taxes: 'Impuestos',
      all_period: 'Todo el Período',
      current_month: 'Mes Actual',
      semester: 'Semestre',
      fiscal_year: 'Año Fiscal',
      custom: 'Personalizado',
      projected_balance: 'Saldo Final Proyectado',
      projected_balance_desc:
        'Saldo proyectado incluyendo el historial filtrado',
      total_incomes: 'Total Ingresos (En Período)',
      total_expenses: 'Total Gastos (En Período)',
      pending_suffix: 'pendientes',
      statement: 'Estado de Transacciones',
      table_date: 'Fecha',
      table_desc: 'Descripción',
      table_type_cat: 'Tipo / Categoría',
      table_status: 'Estado',
      table_amount: 'Monto',
      table_balance: 'Saldo',
      table_actions: 'Acciones',
      monthly: 'Mensual',
      yearly: 'Anual',
      income: 'Ingreso',
      expense: 'Gasto',
      fixed_cost: 'Costo Fijo',
      variable_cost: 'Costo Variable',
      cleared: 'Pagado/Recibido',
      pay: 'Pagar',
      edit_transaction: 'Editar Transacción',
      save_changes: 'Guardar Cambios',
      delete_transaction: 'Eliminar Transacción',
      delete_confirm:
        '¿Está seguro de que desea eliminar esta transacción? Esta acción no se puede deshacer.',
      cancel: 'Cancelar',
      delete: 'Eliminar',
      no_transactions:
        'No se encontraron transacciones para los filtros seleccionados.',
      success_add: 'Transacción añadida exitosamente',
      success_edit: 'Transacción actualizada exitosamente',
      success_delete: 'Transacción eliminada exitosamente',
      success_paid: 'Transacción marcada como pagada',
      invalid_amount: 'Monto inválido',
      invalid_amount_desc: 'El monto debe ser mayor que cero.',
      invalid_desc: 'Descripción inválida',
      invalid_desc_hint: 'La descripción es obligatoria.',
      reconciliation: 'Conciliación',
      upload_statement: 'Cargar Extracto',
      upload_desc: 'Suba extractos bancarios para conciliación.',
      statements: 'Extractos',
      no_statements: 'Ningún extracto cargado.',
      reconciled: 'Conciliado',
      pending_reconciliation: 'Pendiente Conciliación',
      overview_pnl: 'Visión General y PyG',
      tower_breakdown: 'Desglose por Torre',
      channel_analytics: 'Análisis de Canales',
      projected_cash_flow: 'Flujo de Caja Proyectado',
      total_expenses: 'Gastos Totales',
      net_income: 'Ingreso Neto',
      categories: {
        other: 'Otros',
        rent: 'Alquiler',
        maintenance: 'Mantenimiento',
        cleaning: 'Limpieza',
        hoa: 'HOA / Condominio',
        tax: 'Impuestos',
        utilities: 'Servicios Públicos',
      },
    },
    tasks: {
      title: 'Tareas',
      subtitle: 'Gestión de mantenimiento y servicios.',
      board: 'Tablero',
      list: 'Lista',
      in_progress: 'En Progresso',
      approval: 'Aprobación',
      status_wait_owner: 'Esperando Propietario',
      status_wait_pm: 'Esperando Gerente',
      task_title: 'Título de la Tarea',
      assignee: 'Responsable',
      scheduled_date: 'Fecha Programada',
      service_type: 'Tipo de Servicio',
      change_status: 'Cambiar Estado',
      location: 'Ubicación',
    },
    visits: {
      schedule_visit: 'Programar Visita',
      enter_details: 'Ingrese los detalles de la visita a continuación.',
      assign_team: 'Asignar Miembro del Equipo / Socio',
      time: 'Hora',
      reason: 'Motivo de la Visita',
      showing: 'Presentación',
      inspection: 'Inspección',
      client_prefs: 'Preferencias del cliente, códigos de acceso, etc.',
      list_title: 'Lista de Visitas',
      upcoming_past: 'Visitas futuras y pasadas.',
      no_visits: 'Ninguna visita programada.',
    },
    calendar: {
      title: 'Calendario',
      subtitle: 'Visión integrada de eventos y tareas.',
      integrated_view: 'Visión Integrada',
      no_activities: 'Ninguna actividad registrada para hoy.',
    },
    messages: {
      title: 'Mensajes',
      subtitle: 'Centro de comunicación unificado.',
      new_message: 'Nuevo Mensaje',
      select_user: 'Seleccione un Usuario',
      group_team: 'Equipo',
      group_tenants: 'Inquilinos',
      group_owners: 'Propietarios',
      group_partners: 'Socios',
      type_message: 'Escriba su mensaje...',
      online: 'En línea',
      no_contacts: 'Sin contactos.',
      search_contact: 'Buscar contacto...',
      no_conv: 'Ninguna conversación encontrada.',
      new_chat_prompt: 'Iniciar una nueva conversación',
      select_prompt: 'Seleccione un contacto para empezar a chatear.',
      start_chat_desc: 'Iniciar Chat',
    },
    users: {
      title: 'Usuarios',
      subtitle: 'Gestión de usuarios y permisos del sistema.',
      role_label: 'Função',
      registered: 'Registrados',
      registered_desc: 'usuarios registrados en el sistema.',
      permissions: 'Permisos',
      status_active: 'Activo',
      status_pending: 'Pendiente',
      status_blocked: 'Bloqueado',
      approve_success: 'Usuario aprobado con éxito.',
      block_title: 'Bloquear Usuario',
      block_confirm: '¿Estás seguro de que deseas bloquear a este usuario?',
      block_success: 'Usuario bloqueado con éxito.',
    },
    workflows: {
      title: 'Flujos de Trabajo',
      subtitle: 'Automatización de procesos y tareas.',
      new_workflow: 'Nuevo Flujo de Trabajo',
      trigger: 'Desencadenante',
      steps: 'Pasos',
      run_manual: 'Ejecutar Manualmente',
      config_workflow: 'Configurar Flujo',
      edit_workflow: 'Editar Flujo',
      active_status: 'Estado Activo',
      step_name: 'Nombre del Paso',
      step_role: 'Función Responsable',
      add_step: 'Añadir Paso',
      run_workflow: 'Ejecutar Flujo',
      select_properties: 'Seleccionar Propiedades',
      tasks_generated_desc:
        'Se generarán tareas para cada propiedad seleccionada.',
    },
    renewals: {
      title: 'Renovaciones',
      subtitle: 'Gestión de vencimiento de contratos.',
      negotiation_started: 'Negociación Iniciada',
      search_placeholder: 'Buscar renovación...',
      current_value: 'Valor Actual',
      suggestion: 'Sugerencia ($)',
      all_status: 'Todos los Estados',
      critical_status: 'Crítico (≤ 30 días)',
      warning_status: 'Aviso (31 - 60 días)',
      safe_status: 'Seguro (> 60 días)',
      days_left: 'quedan {days} días',
      current_proposed: 'Actual / Propuesto',
      negotiation_status: 'Estado de Negociación',
      renewed: 'Renovado',
      owner: 'Propietario',
      tenant: 'Inquilino',
      manage: 'Gestionar',
      no_results: 'Ninguna renovación coincide con los filtros.',
      negotiation_hub: 'Centro de Negociación',
      negotiation_hub_desc:
        'Gestione renovaciones de alquiler de manera eficiente.',
      confidential_note: 'Confidencial: Visible solo para Gerentes.',
      add_note: 'Añadir Nota Interna',
      mark_vacating: 'Marcar como Desalojando',
      close_negotiation: 'Cerrar Negociación',
      close_negotiation_desc:
        'Confirme los nuevos términos y adjunte el contrato firmado para finalizar. El estado será "Renovado".',
      new_value: 'Nuevo Valor de Alquiler',
      new_start_date: 'Nueva Fecha Inicio',
      new_end_date: 'Nueva Fecha Fin',
      contract_upload: 'Subir Contrato',
      fill_all_fields:
        'Por favor, rellene todos los campos y adjunte el contrato.',
      both_must_accept:
        'Ambos, Propietario e Inquilino, deben aceptar antes de cerrar.',
      contract_renewed: 'Contrato renovado con éxito.',
      pricing_owner_approval: 'Precios y Aprobación del Propietario',
      proposed_value: 'Valor Propuesto',
      update_terms_notify: 'Actualizar Términos y Notificar Propietario',
      pricing_tenant_approval: 'Precios y Aprobación del Inquilino',
      new_rent_value: 'Nuevo Valor de Alquiler',
      pending_owner_approval: 'Aprobación del Propietario Pendiente',
      save_decision: 'Guardar Decisión',
      type_message: 'Escriba un mensaje...',
      empty_notes: 'Ninguna nota aún.',
      empty_messages: 'Ningún mensaje aún.',
    },
    short_term: {
      title: 'Alquiler Temporal',
      subtitle: 'Gestión de reservas a corto plazo.',
      new_booking: 'Nueva Reserva',
      guest: 'Huésped',
      check_in: 'Check-in',
      check_out: 'Check-out',
      total: 'Total',
      platform: 'Plataforma',
      reports: 'Informes',
      calendar: 'Calendário',
      bookings: 'Reservas',
      communication: 'Comunicación',
      revenue: 'Ingresos',
      occupancy: 'Ocupación',
      auto_replies: 'Respuestas Automáticas',
      send_welcome: 'Enviar Mensaje de Bienvenida',
      send_checkout_instr: 'Enviar Instrucciones de Salida',
      template_name: 'Nombre de la Plantilla',
      delay_minutes: 'Retraso (minutos)',
      amount: 'Importe',
      direct_booking: 'Directo / Manual',
      full_name_placeholder: 'Nombre Completo',
      email_placeholder: 'email@huesped.com',
    },
    common: {
      add_document: 'Añadir Documento',
      document_name: 'Nombre del Documento',
      file_url: 'URL del Archivo',
      no_documents: 'Ningún documento aún.',
      personal_data: 'Datos Personales',
      contact_address: 'Contacto y Dirección',
      documents: 'Documentos',
      rg_id: 'Documento de Identidad',
      dob: 'Fecha de Nac. / Fundación',
      nationality: 'Nacionalidad',
      marital_status: 'Estado Civil',
      profession: 'Profesión',
      monthly_income: 'Ingreso Mensual',
      base_rent: 'Alquiler Base',
      secondary_phone: 'Teléfono Secundario',
      whatsapp: 'WhatsApp',
      street: 'Calle/Avenida',
      number: 'Número',
      complement: 'Número o complemento',
      neighborhood: 'Barrio',
      city: 'Ciudad',
      state: 'Estado',
      zip_code: 'Código Postal',
      single: 'Soltero(a)',
      married: 'Casado(a)',
      divorced: 'Divorciado(a)',
      widowed: 'Viudo(a)',
      country_us: 'Estados Unidos',
      country_br: 'Brasil',
      country_es: 'España',
      tax_id_label: 'NIF / RUT / Tax ID',
      full_name_label: 'Nombre Completo / Razón Social',
      add: 'Añadir',
      dashboard: 'Panel Inmobiliario',
      real_estate_dashboard: 'Panel Inmobiliario',
      properties: 'Propiedades',
      tenants: 'Inquilinos',
      owners: 'Propietarios',
      partners: 'Socios',
      calendar: 'Calendario',
      tasks: 'Tareas',
      financial: 'Financiero',
      financial_hoa: 'Financiero HOA',
      invoices: 'Facturas',
      messages: 'Mensajes',
      settings: 'Configuraciones',
      service_pricing: 'Catálogo de Preços',
      search: 'Buscar...',
      filter: 'Filtrar',
      status: 'Estado',
      actions: 'Acciones',
      save: 'Guardar',
      cancel: 'Cancelar',
      edit: 'Editar',
      delete: 'Eliminar',
      new: 'Nuevo',
      back: 'Volver',
      view: 'Ver',
      details: 'Detalles',
      name: 'Nombre',
      email: 'Correo electrónico',
      phone: 'Teléfono',
      address: 'Dirección',
      type: 'Tipo',
      role: 'Cargo',
      role_label: 'Función',
      logout: 'Cerrar sesión',
      profile: 'Perfil',
      notifications: 'Notificaciones',
      team: 'Equipo',
      automation: 'Automatización',
      all: 'Todos',
      pending: 'Pendiente',
      accepted: 'Aceptado',
      rejected: 'Rechazado',
      counter: 'Contraoferta',
      approved: 'Aprobado',
      completed: 'Completado',
      paid: 'Pagado',
      overdue: 'Vencido',
      active: 'Activo',
      inactive: 'Inactivo',
      occupied: 'Ocupado',
      vacant: 'Vacante',
      maintenance: 'Mantenimiento',
      cleaning: 'Limpieza',
      confirm: 'Confirmar',
      upload: 'Subir',
      description: 'Descripción',
      date: 'Fecha',
      end_date: 'Fecha Fin',
      due_date: 'Vencimiento',
      value: 'Valor',
      priority: 'Prioridad',
      yes: 'Sí',
      no: 'No',
      none: 'Ninguno',
      other: 'Otro',
      error: 'Error',
      condominiums: 'Condominios',
      continue: 'Continuar',
      password: 'Contraseña',
      confirm_password: 'Confirmar Contraseña',
      add_title: 'Añadir',
      contact_via_whatsapp: 'WhatsApp',
      contact_via_email: 'Correo',
      tax_id: 'NIF/Tax ID',
      system_activity: 'Actividad del Sistema',
      approve: 'Aprobar',
      reject: 'Rechazar',
      block: 'Bloquear',
      pending_approval: 'Pendiente Aprobación',
      pending_activation: 'Pendiente Activación',
      blocked: 'Bloqueado',
      country: 'País',
      select_country: 'Seleccione el País',
      export_data: 'Exportar Datos',
      export_success: 'Datos exportados con éxito.',
      export_success_title: 'Exportación Iniciada',
      renewals: 'Renovación de Contrato',
      short_term: 'Alquiler Temporal',
      market_analysis: 'Análisis de Mercado',
      advanced_analytics: 'Análisis Avanzado',
      automation_rules: 'Reglas de Automatización',
      workflows: 'Flujos de Trabajo',
      portal: 'Portal',
      publicity: 'Publicidad',
      login: 'Entrar',
      register: 'Registrarse',
      welcome: 'Bienvenido',
      full_name: 'Nombre Completo',
      property: 'Propiedad',
      reports: 'Informes',
      delete_title: '¿Estás seguro?',
      delete_desc: 'Esta acción no se puede deshacer.',
      start_now: 'Empezar Ahora',
      download: 'Descargar',
      print: 'Imprimir',
      close: 'Cerrar',
      delete_active_tenant_error:
        'No es posible eliminar propiedad con inquilino activo.',
      confirm_delete: 'Confirmar Eliminación',
      operations: 'Operaciones',
      contracts: 'Contratos',
      total: 'Total',
      done: 'Concluir',
      taxes: 'Impuestos',
      utilities: 'Servicios',
      inventory: 'Inventario',
      history: 'Historial',
      activity_log: 'Registro de Actividad',
      created_at: 'Creado en',
      updated_at: 'Actualizado en',
      select: 'Seleccione',
      required: 'Obligatorio',
      success: 'Éxito',
      removed: 'Eliminado',
      copied: 'Copiado',
      empty: 'Vacío',
      relationships: 'Relaciones',
      relationship_owner: 'Propietario',
      relationship_tenant: 'Inquilino',
      no_owner: 'Sin Propietario',
      add_owner: 'Añadir Propietario',
      no_active_tenant: 'Sin Inquilino Activo',
      export: 'Exportar',
      visits: 'Visitas',
      schedule_visit: 'Programar Visita',
      schedule_maintenance: 'Programar Mantenimiento',
      client_name: 'Nombre del Cliente',
      loading: 'Cargando...',
      unknown: 'Desconocido',
      resource: 'Recurso',
      profile: 'Perfil',
      visit_date: 'Fecha de la Visita',
      visit_status: 'Estado de la Visita',
      scheduled: 'Programada',
      canceled: 'Cancelada',
      visit_scheduling: 'Programación de Visitas',
      visits_list: 'Lista de Visitas',
      total_revenue: 'Ingresos Totales',
      sold: 'Vendido',
      sale_pending: 'Venta Pendiente',
      category: 'Categoría',
      quantity: 'Cantidad',
      condition: 'Condición',
      revenue: 'Ingresos',
      profit: 'Beneficio',
      provider: 'Proveedor',
      analytics: {
        benchmark_title: 'Benchmark de Mercado',
        benchmark_desc: 'Comparativo de rendimiento con el mercado.',
        house_model: 'Modelo de Casa',
        internal_perf: 'Rendimiento Interno',
        market_avg: 'Promedio de Mercado',
        occupancy: 'Ocupación',
        profitability_title: 'Rentabilidad',
        profit: 'Beneficio',
        revenue: 'Ingresos',
        adr: 'Tarifa Media Diaria (ADR)',
        privacy_mode: 'Modo Privacidad',
      },
      help_hub: 'Centro de Ayuda',
      hotels: 'Hoteles',
      location: 'Ubicación',
      access_credentials: 'Credenciales de Acceso',
      manager: 'Gerente',
      contact: 'Contacto',
      history_adjustments: 'Historial de Ajustes',
      name_required: 'El nombre es obligatorio',
      email_invalid: 'Correo electrónico inválido',
      error_404_title: '404',
      error_404_desc: '¡Ups! Página no encontrada.',
      return_home: 'Volver al Inicio',
      access_denied: 'Acceso Denegado',
      access_denied_desc: 'No tienes permiso para ver esta página.',
      list: 'Lista',
      calendar_view: 'Calendario',
      start_date: 'Fecha Inicio',
      and: 'y',
      validation_error: 'Error de Validación',
      delete_success: 'Eliminado con éxito.',
      error_delete: 'Error al eliminar.',
      delete_linked_error:
        'No es posible eliminar porque existen registros vinculados.',
      new_task: 'Nueva Tarea',
      create_title: 'Crear Tarea',
      search_address: 'Buscar Dirección',
      manager_col: 'Gerente',
    },
  },
}
